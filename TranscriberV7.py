...
def _(call, np, parselmouth, pd):
    # =====================================================================
    # CELL 9 — PER-WORD PROSODY EXTRACTOR (+ f0 slope, new in V7)
    # =====================================================================
    def extract_word_features(audio_path, word_segments):
        ...
        for i, w in enumerate(word_segments):
            ...
            # --- f0 SLOPE (new) ---
            # Fit a straight line through the word's pitch contour.
            # Positive = the voice RISES across the word, negative = it FALLS.
            # Measured in semitones per second, not Hz per second, so it is:
            #   speaker-independent — a low male voice and a high female voice
            #     rising by the same musical interval produce the same number
            #   duration-independent — a slow rise and a fast rise over the
            #     same interval are distinguishable
            # This is the signal the MOTION channel uses.
            if len(f0v) >= 3:
                _t = np.linspace(0.0, max(duration, 1e-6), len(f0v))
                _semitones = 12.0 * np.log2(f0v / f0v[0])   # relative to word onset
                f0_slope = float(np.polyfit(_t, _semitones, 1)[0])
            else:
                f0_slope = 0.0     # unvoiced, or too few frames to fit a line
...
def _(mo):
    ...
    - **Every word re-declares every tag it uses.** .ass override tags persist
      ...
      why `\fscx100\fscy100\frz0` appears in every word's tag block.
    - **`\fscy` swell is the default motion**, because it cannot break the
      ...
      `\frz` tilt is more literal but rotation propagates aggressively along
...
def _(mo):
    ...
    8. **Swell (\fscy) is the default gesture, tilt (\frz) is opt-in.**
       .ass has no per-word position tag (\pos and \move are line-level, like
       \fad), so a "lift" must be built from scale or rotation. Swell cannot
       ...
       every subsequent word resets \frz0. Both are implemented; the safe one
       is the default.
