...
def _(mo):
    mo.md(r"""
    # V1.0: the budget gets an absolute floor, and shouting becomes measurable

    Two additions, both prompted by watching real footage rather than by
    reading anything.

    **A screamed line was rendering at the smallest type in the system.** This
    is the boundary of the expressive budget's own best idea. Because a word's
    intensity is its *share* of a fixed hundred points, a segment where every
    word is shouted gives every word a fair share, so every share ratio sits at
    about 1.0 and every intensity comes out near zero. The angriest delivery in
    a clip therefore rendered flattest. A gain cannot fix it, because any
    multiplier on zero is still zero. Each segment's *absolute* loudness and
    pitch now set a floor that the whole line renders at, with the
    within-line competition riding on top of it.

    **Capitals now come from a measurement.** The DCMP Captioning Key reserves
    capitals for screaming or shouting and forbids them for general emphasis —
    the one style choice in this project that already has a published rule
    attached. Human captioners apply it by listening. Here it is applied from
    spectral tilt: the ratio of energy above 1 kHz to energy below it, which
    rises with vocal effort and is gain-invariant, so it can tell a shout apart
    from a close microphone or a hot recording. Anger and shouting are different
    things — people shout when happy and are quietly furious — so the gate reads
    vocal effort, not the predicted emotion and not loudness alone.

    Two smaller things came out of the same sessions. Every output filename is
    now built from one version string instead of three hardcoded literals, and
    the CUDA check allocates a tensor to force a context rather than trusting
    `is_available()`, so a card that reports free but is full fails at startup
    with something actionable instead of halfway through a batch.
    """)
    return
...
def _(
    ...
    # Note: hand torch's cache back to the driver BEFORE CTranslate2 asks
...
def _():
    ...
    SEGMENT_AROUSAL_FLOOR = 0.45     # 0.0 = V0.8-9 behaviour (no floor)
    ...
    YELL_Z = 1.30              # z of the effort score, ACROSS segments
...
def _(classify_words, np, pd):
    ...
    def detect_yelling(words_df, features, z_thresh, max_frac=0.25,
        ...
        for feat, w in features.items():
            ...
            if np.all(np.isnan(means)):
                continue
            _c, _s = center_scale(means[~np.isnan(means)], robust, rel_floor=0.02)
            if _s <= eps:
                continue
    ...
    def yell_report(words_df, features, z_thresh, tilt_min_z=None,
                    min_db_over_median=0.0, top=8, robust=True, eps=1e-9):
        ...
        def _segz(feat):
            ...
            if np.all(np.isnan(m)):
                return None
            _c, _s = center_scale(m[~np.isnan(m)], robust, rel_floor=0.02)
            if _s <= eps:
                return None
            return np.where(np.isnan(m), 0.0, (m - _c) / _s), m
        ...
        rep["score_ok"] = rep["score_z"] > z_thresh
        ...
        print(f"YELL DIAG: thresholds score>{z_thresh}, tilt>{tilt_min_z}, "
              f"dB>=+{min_db_over_median}. Loudest segments:")
        print(rep.to_string(index=False))
        if not rep["would_fire"].any():
            _blocked = []
            if not rep["score_ok"].any():
                _blocked.append("combined score (lower YELL_Z)")
...
def _(
    ...
    if YELL_DETECT:
        budget_df["is_yell"], budget_df["yell_z"] = detect_yelling(
            ...
            min_db_over_median=YELL_MIN_DB_OVER_MEDIAN, robust=ROBUST_STATS)
    else:
...
def _(
    ...
    def process_any_video(video_path, out_tag=None, use_bg_video=True):
        ...
        aligned = whisperx.align(result["segments"], align_model, align_meta, audio, device,
                                 return_char_alignments=False)
        # Note: this function loads a SECOND alignment model on top of the
...
def _(mo):
    ...
    1. **The budget needed an absolute floor.** A word's intensity is its share
        of a fixed hundred points, which makes emphasis relative and scarce by
        construction — the whole point of the design. But it also means a segment
        where every word is shouted gives every word a fair share, so every
        share ratio is about 1.0 and every intensity lands near zero: the
        angriest line in a clip rendered at the smallest type in the system. A
        gain would not fix it, because any multiplier on zero is still zero. Each
        segment's absolute loudness and pitch are now z-scored across the clip
        and set a floor the whole line renders at, via
        `intensity = floor + (1 - floor) * intensity_raw`. That is monotone in
        the raw value, so the ordering of words inside a line is untouched, and
        it is bounded by one. Setting `SEGMENT_AROUSAL_FLOOR = 0.0` reproduces
        the previous behaviour exactly, which makes this an A/B condition rather
        than a one-way change.

    2. **The floor is deliberately emotion-agnostic.** It fires on loud speech
        whatever the classifier called it, so it cannot give one emotion more ink
        than the others in an evaluation. `EMOTION_FLOOR_BONUS` exists as a dial
        and ships at all zeros: it is a taste parameter, not a measured one, and
        weighting classes by hand is exactly the move the audit warned about.

    3. **Capitals are triggered by spectral tilt, not by loudness or emotion.**
        The DCMP Captioning Key reserves capitals for screaming or shouting and
        forbids them for general emphasis, which makes this the one style choice
        in the project with a published rule already attached. The new
        `alpha_ratio` feature is the ratio of energy above 1 kHz to energy below
        it, in dB. It rises with vocal effort and is gain-invariant, so unlike
        level alone it can distinguish a shout from a close microphone. The gate
        reads effort rather than salience, loudness alone, or the predicted
        emotion, because anger and shouting are different things: people shout
        when happy and are quietly furious.

    4. **The shout gate is conservative on purpose, and the reason is
        legibility.** Capitals strip the ascender and descender cues that
        word-shape recognition uses, and the same guidance separately requires
        fonts that *have* descenders — so a caption in permanent capitals would
        violate the standard it is citing. Hence a high z threshold, a second
        independent gate on the tilt term, an absolute dB margin over the
        segment median rather than a relative z alone, a cap on what fraction of
        a clip may be recased, and a reading-rate penalty because all-caps reads
        slower. Punctuation may lower the threshold but never cross it alone: a
        transcribed exclamation mark is a language-model guess, not a
        measurement.

    5. **The recased text goes in a separate column, set before measurement.**
        `word_display` carries the capitals and `word` stays the faithful
        transcript, so nothing downstream can accidentally treat the styling as
        the transcript. It is assigned before text measurement rather than at
        render time, because the absolute layout sizes each word's box from the
        display string — changing the text after measuring it would desync the
        burned-in caption from its own geometry.

    6. **A failed spectrum is reported once, loudly.** If the Praat spectrum step
        fails, `alpha_ratio` is zero for every word and the detector silently
        falls back to level, which cannot tell a shout from a loud recording.
        Swallowing that is how a feature ends up dead while appearing to work, so
        it prints once with the consequence spelled out. `yell_report()` exists
        for the same reason: "no capitals appeared" has at least four causes that
        look identical from outside — the tilt feature never computed, the
        combined score fell short, the tilt gate blocked, or the dB margin
        blocked — and guessing between them by nudging thresholds is how a
        conservative detector gets loosened into a broken one. It prints the
        evidence instead.

    7. **One version string drives every output name.** It was three separate
        hardcoded literals, which is why some renders were still labelled with a
        two-version-old tag and any video run was tagged "anyvideo". A stale
        filename on a stimulus is not cosmetic: it is how the wrong render ends
        up in a study.

    8. **The CUDA check allocates rather than asks.** `torch.cuda.is_available()`
        returns True for a card that is present but full. The probe now
        allocates a small tensor to force a CUDA context and reads the real free
        memory, so a full card fails at startup with an actionable message
        instead of halfway through a batch. `FORCE_DEVICE` pins it either way,
        and the resolved device is printed with the reason.

    ### Carried forward from V0.8-9

    9. **Motion is isolated by absolute layout, not by attenuation.** V0.7
        ...
        smaller, not fixed it. V0.8-9 measures each word with the font file
    ...
    10. **Emphasis is directional.** V0.7 scored `w * |z|`, which treated a word
        ...
        V0.8-9 clips the emphasis features at zero so only above-average loudness,
        pitch and lengthening earn points. HNR stays two-sided, because creak
        and breathiness are both marked voice qualities.

    11. **Duration is a residual, not a raw value.** Raw duration conflates
        "this word has four syllables" with "the speaker leaned on this word".
        V0.8-9 fits duration against syllable count across the clip and scores the
    ...
    12. **Pitch is tracked once, with a speaker-adapted range.** V0.7 re-ran
        ...
        single halved frame into an enormous apparent range. V0.8-9 tracks the
    ...
    13. **A word-class prior, with a prosodic override.** Determiners,
    ...
    14. **An emphasis quota.** Even with correct scoring, a line where a third
    ...
    15. **ASR quality is a model-size problem first.** V0.7 ran whisper "base"
        with no language hint. Size dominates WER, so V0.8-9 defaults to large-v3
    ...
    16. **Palette hues are chosen by measurement, not by eye.** A CIEDE2000
        ...
        hue-crowding problem and the CVD problem were the same problem. V0.8-9's
    ...
    17. **Fearful's arousal inversion is fixed in both channels it appeared
        in.** V0.8-9 encoded fear at saturation 0.28 / value 0.88, which the
        ...
        high-arousal states). V0.8-9: (0.70, 0.78) → +0.18, above sad, and a new
    ...
    18. **The colour science self-checks or refuses to run.** ΔE2000 follows
    ...
    19. **The channel rule is resolved as an explicit, evidenced switch.**
    ...
    20. **The confidence curve stops overstating the classifier.** The legacy
    ...
    21. **The swell is volume-conserving squash-and-stretch.** Every `\fscy`
    ...
    22. **Reading rate is enforced, bounded, and reported.** Broadcast caption
        practice works to a characters-per-second ceiling; V0.8-9 extends a
    ...
    23. **Legibility parameters are visible, and substitution is a failure.**
    ...
    24. **The adjective/adverb tier ships off.** EmoLex's POS breakdown
    ...
    25. **The saturation floor is a named trade-off.** At intensity 0 every
