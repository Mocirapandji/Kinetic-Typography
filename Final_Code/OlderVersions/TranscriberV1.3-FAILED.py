...
@app.cell
def _():
    # V1.3 — ABANDONED BRANCH. See the markdown cell below.
...
def _(mo):
    mo.md(r"""
    # V1.3 — ABANDONED. Kept for the record, not for use.

    > **Do not run this as the deliverable.** This branch and a parallel
    > reorganisation attempt were both made after V1.2 and neither was merged.
    > V1.2 remains the shipped version. This file exists so the two experiments
    > and their reasons for failing are on the record rather than lost.

    ## What this branch tried

    Both of V1.2's stimulus failures were traced to the same root cause seen
    from two directions: tokens the tooling cannot handle. `LOOP_WORD_RE` strips
    digits, which is why `$672,000` tokenised to nothing and a real line got
    deleted as a hallucination. The forced aligner has the same problem for a
    different reason — it has no phonemes for a numeral or a currency symbol, so
    those tokens align badly or not at all, and every prosodic measurement
    downstream sits inside boundaries the aligner produced.

    So this branch added a sanitisation layer around alignment:

    - `sanitize_for_alignment` swaps any token containing a digit or currency
      symbol for a length-matched run of `q`, queueing the originals, so the
      aligner sees something it can pronounce and the duration estimate stays
      roughly right.
    - `restore_after_alignment` puts the originals back from that queue.
    - `presplit_long_segments` cuts segments over six seconds on sentence
      boundaries, apportioning time by character length with padding either
      side, on the theory that the aligner does better on shorter spans.
    - `smart_align` wraps presplit → sanitize → align → restore into one call so
      the batch path and the single-clip path share an implementation.

    ## Why it was not merged

    The sanitisation is not wrong in principle, but three properties of this
    implementation make it risky enough that shipping it unverified would have
    been worse than the problem it solves.

    **The restore is positionally fragile.** `restore_after_alignment` pops from
    a file-wide FIFO queue in order. If the aligner *drops* any placeholder —
    which is the exact failure this layer exists to prevent, so it cannot be
    assumed not to happen — every subsequent restoration shifts by one and
    numbers get silently reattached to the wrong words. There is no index, no
    length check, and no reconciliation against the queue at the end. A caption
    that says the wrong number is worse than one that mis-times a right one,
    especially for a viewer who cannot hear the audio to catch it.

    **`presplit_long_segments` drops metadata.** It emits dicts carrying only
    `start`, `end` and `text`, so `speaker`, `words` and anything else attached
    to the segment are lost. Speaker labels are read off segments in two places
    for the smoother's boundary resets.

    **The padding makes adjacent pieces overlap.** Each piece is extended by a
    second on both sides, so consecutive pieces can overlap by up to two
    seconds. Overlapping spans fed to the aligner can yield duplicate words, and
    `assign_words_to_segments` resolves a word to the *first* window containing
    its midpoint, so the overlap region is assigned arbitrarily.

    Each of these produces symptoms that look like an alignment bug rather than
    a sanitisation bug, which makes them expensive to chase.

    ## The other abandoned attempt

    A parallel branch reorganised the notebook into eight thematic sections
    instead of numbered cells. It changed no parameter values at all — the
    behaviour was identical — but it stripped roughly nine-tenths of the
    comments in the process, taking with them every reference to the sources the
    palette, the arousal ramp and the capitalisation rule are built on, and the
    record of which parameter values were measured versus chosen. It also left
    references to the cell numbers it had just removed, deleted the strictest
    control condition from the evaluation batch, and reverted the version tag.
    The sectioning itself is a reasonable idea and could be ported back onto
    V1.2 with the commentary intact; nothing else from it is worth keeping.

    ## What is worth taking from here

    The gram-size correction in `looks_like_loop`. The repeating-unit length is
    derived from the token count rather than hardcoded, which is what allows a
    four-word cycle to be detected at all — a fixed set of short gram sizes was
    sized to the phrases seen so far and cannot catch a longer one, because
    chopping a four-word unit into three-word grams shifts the phase on every
    repetition so no run of identical grams ever forms. That change is
    self-contained and carries none of the risks above.
    """)
    return
...
def _(os, torch):
    ...
    #
    # one tag drives every output name. The code has run ahead of the tag since the strict/tiered
...
def _():
    ...
    ASR_DROP_LOOPS = True
    # 4 -> 6. BreakingBad(Happy) transcribed as
...
def _(RandomForestClassifier, clip_df, joblib, os):
    ...
    # ---------------------------------------------------------------------
    # The previous version hardcoded clf_v2.joblib and read exactly 4 keys, which
    ...
    MODEL_BUNDLE_CANDIDATES = [
        "outputs/clf_v3.joblib",      # IEMOCAP-primary, RAVDESS support
        "outputs/clf_v2.joblib",      # legacy, kept so old runs reproduce
    ]
...
def _(
    ...
    def enforce_min_dwell(path, seg_df, min_dwell_s, S, speaker_ids=None):
        ...
        `speaker_ids`, if given (a per-segment array aligned with
    ...
    def smooth_segment_emotions(seg_df, classes, mode="viterbi",
        ...
        * It cannot invent a label. Viterbi picks from what the classifier
        ...
        After the path is decoded, enforce_min_dwell runs as a second,
        ...
        `speaker_ids`, a dict {segment_id: speaker_label} from
        ...
        the earlier behaviour exactly (uniform persistence, no boundaries).
        ...
        if not all(c in out.columns for c in cols):
            print("SMOOTH: no posterior columns — these predictions predate "
                  "an older run. Re-run prediction; returning unsmoothed.")
            out["pred_emotion_raw"] = out["pred_emotion"]
            out["switched_by_smoother"] = False
            return out
...
def _(
    ...
    # minimum share of a segment the repeated gram must occupy
    ...
    def looks_like_loop(text, min_repeats=6, min_coverage=ASR_LOOP_MIN_COVERAGE):
        ...
        this branch -- one more change, forced by the CELL 9e self-test, which
        this function still failed: it never caught
        "Subscribe to my channel." repeated six times.

        The previous loop only tried gram sizes (1, 2, 3). "Subscribe to my
        channel." is a 4-word repeating unit, and 4 does not divide into
        chunks of 1, 2, or 3 in a way that keeps consecutive grams
        identical -- chop a 4-word cycle into 3-word pieces and the phase
        shifts on every repetition, so no run of identical grams ever
        forms. The hardcoded (1, 2, 3) was never a general answer to "how
        long is the repeating unit", it was sized to the phrases seen so
        far ("Thank you." at 2, "you" at 1).

        NEW: the gram sizes to try are now derived from the data. The
        largest unit that could possibly repeat min_repeats times in a
        token list of this length is len(toks) // min_repeats, so every
        size from 1 up to that bound is checked -- 4-word, 5-word, or
        longer repeating phrases are no longer structurally invisible.

        The two earlier changes are unchanged and still do their job:

        1. min_repeats 4 -> 6. Genuine Whisper loops run 8-20+ repeats.

        2. min_coverage -- a real hallucination loop IS the segment;
           genuine repetition is EMBEDDED in other speech ("$672,000...
           Each. Each. Each. $672,000 each. Each." is 5 "each"es in 23
           tokens, well under min_repeats and under min_coverage either
           way). Requiring the run to be both long AND most of the
           segment is what keeps that line from being screened out.
        ...
        if len(toks) < min_repeats:
            return False
        # NEW: try every unit size that could possibly repeat
        # min_repeats times, not a hardcoded (1, 2, 3).
        ...
        for size in range(1, max_size + 1):
            ...
            for a, b in zip(grams, grams[1:]):
                ...
                best = max(best, run)
            # the run must be long AND must be most of the segment
...
@app.cell
def _(re):
    # CELL 8b — SANITIZE UNALIGNABLE CHARACTERS + SPLIT LONG SEGMENTS
    #           BEFORE FORCED ALIGNMENT
    _UNALIGNABLE_RE = re.compile(r'[\d£$€¥]')

    def sanitize_for_alignment(segments):
        placeholder_queue = []
        out_segments = []
        for seg in segments:
            new_words = []
            for w in seg["text"].split():
                if _UNALIGNABLE_RE.search(w):
                    placeholder_queue.append(w)
                    new_words.append("q" * max(2, len(w)))
                else:
                    new_words.append(w)
            out_segments.append({**seg, "text": " ".join(new_words)})
        return out_segments, placeholder_queue

    def restore_after_alignment(aligned, placeholder_queue):
        _q = list(placeholder_queue)
        for w in aligned["word_segments"]:
            if re.fullmatch(r"[Qq]+\.?", str(w.get("word", ""))) and _q:
                w["word"] = _q.pop(0)
        return aligned

    _SENTENCE_END_RE = re.compile(r'(?<=[.!?])\s+')

    def presplit_long_segments(segments, max_segment_s=6.0, pad_s=1.0):
        out = []
        for seg in segments:
            ...
            if dur <= max_segment_s:
                out.append(seg)
                continue
            pieces = [p for p in _SENTENCE_END_RE.split(seg["text"].strip()) if p]
            if len(pieces) < 2:
                out.append(seg)
                continue
            ...
            for p, l in zip(pieces, lens):
                piece_dur = dur * (l / total_len)
                padded_start = max(seg["start"], t - pad_s)
                padded_end = min(seg["end"], t + piece_dur + pad_s)
                out.append({"start": padded_start, "end": padded_end, "text": p})
                t += piece_dur
        return out

    return (
        presplit_long_segments,
        restore_after_alignment,
        sanitize_for_alignment,
    )
...
def _(
    ...
    def smart_align(segments, align_model, align_metadata, audio, device,
                    max_segment_s=6.0, pad_s=1.0, return_char_alignments=False):
        """Presplit + sanitize + align + restore, in one call.

        Exactly what CELL 9 does inline for the single-clip test path;
        process_any_video calls this instead of duplicating those four
        steps in the batch/production path.
        """
        segments = presplit_long_segments(segments, max_segment_s=max_segment_s, pad_s=pad_s)
        segments, placeholder_queue = sanitize_for_alignment(segments)
        aligned = whisperx.align(segments, align_model, align_metadata, audio, device,
                                 return_char_alignments=return_char_alignments)
        aligned = restore_after_alignment(aligned, placeholder_queue)
        return aligned
...
def _(
    ...
    result["segments"] = presplit_long_segments(result["segments"], max_segment_s=6.0, pad_s=1.0)
    result["segments"], _placeholder_queue = sanitize_for_alignment(result["segments"])
    aligned = whisperx.align(result["segments"], align_model, align_metadata, audio, device,
                             return_char_alignments=False)
    aligned = restore_after_alignment(aligned, _placeholder_queue)
...
def _(call, np, parselmouth, pd):
    ...
    def extract_word_features(audio_path, word_segments, count_syllables_fn):
        ...
        if df.empty:
            # an empty frame here is what turned a dropped segment
...
def _(os):
    ...
    # BACK TO STRICT, AND TIERED (reverts the earlier widening)
    # ---------------------------------------------------------------------
    # An earlier pass shipped EXCLAIM_EMOTIONS = {angry, frustrated, excited, happy} at
    ...
    # was the single biggest source of stray capitals in the earlier pass.
    #
    ...
    # including it is most of what made the earlier pass shout constantly. Someone
    ...
    # SHOUT SENSITIVITY, AS A NAMED PRESET
    # ---------------------------------------------------------------------
    ...
    # "strict" is the default and the new entry in the table.
    ...
    # ----- the "super super" bar for SHOUT_EXTREME_EMOTIONS -------
    ...
    # ----- the torn / "close to mad" clause, now OFF --------------
    ...
    # ----- hard cap on how much of a clip may shout ---------------
    ...
    # ----- does the ACOUSTIC yell detector get an emotion veto? ---
    ...
    # ----- CHANNEL 4a: MERGE SEGMENTS TOO SHORT TO READ ----------
    ...
    # ----- CHANNEL 4b: SILENCE AS PUNCTUATION AND AS A LINE BREAK ---
    ...
    # TWO NEW CLASSES: frustrated, excited
    # ---------------------------------------------------------------------
    ...
    EMOTION_STYLES = {
        ...
        "frustrated": {"h": 0.0556, "s": 0.70, "v": 0.62, "i": 0, "anim": "flat",   "font": "DejaVu Sans Condensed"},  # 20deg rust
        "happy":      {"h": 0.1400, "s": 0.90, "v": 1.00, "i": 0, "anim": "pop",    "font": "DejaVu Sans"},
        "excited":    {"h": 0.8750, "s": 0.85, "v": 1.00, "i": 0, "anim": "pop",    "font": "DejaVu Sans"},             # 315deg magenta
...
def _(
    ...
    assert _arousal["fearful"] > _arousal["sad"], (
        ...
        "the fear bug (fear at -0.10 vs sad +0.08) made permanent")
...
def _(
    ...
    cvd_fig.suptitle("Palette under colour-vision-deficiency simulation "
...
def _(colorsys, np, re):
    ...
    def apply_emotion_shout(styled_df, emotions, blend_margin,
                            ...
                            yell_emotion_gate=True):
        """A SECOND, emotion-driven shout trigger, independent of
        ...
        # both floors are CHANCE-RELATIVE, driven by the
        ...
        # report the FIRE RATE. "Too trigger happy" and "never fires"
        ...
        # when nothing fires, say WHICH gate stopped what -- the same
    ...
    # ---------------------------------------------------------------------
    # MERGE SEGMENTS TOO SHORT TO READ
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # SILENCE AS PUNCTUATION, AND AS A LINE BREAK
    # ---------------------------------------------------------------------
    ...
    def split_on_long_pauses(df, enable=True, dots_min_s=0.28,
        ...
        Runs AFTER merge_rapid_segments, so the ids it reads are
...
def _(font_vmetrics, pause_gap, space_width, text_width):
    ...
    # ---- squash-and-stretch geometry (audit 4.3) --------------------
    ...
    # a squashed word will occupy. these were 0.6/0.5/0.4 as magic numbers in
    ...
    def layout_segment(seg_rows, width, height, margin_x, margin_v,
        ...
        for _, rw in seg_rows.iterrows():
            ...
            # horizontal headroom for the squash (see swell_headroom_x)
...
def _(
    ...
    if USE_CONFIDENCE_SCALING:
        # calibrated curve from cell 12b (0.43 -> ~0.60, not ~0.90)
...
def _(classify_words, np, pd):
    ...
    def compute_salience(words_df, weights, zero_missing=frozenset(),
        ...
        The empty-frame guard. BreakingBad(Happy) reached here with
    ...
    def yell_report(words_df, features, z_thresh, tilt_min_z=None,
        ...
        if "alpha_ratio" not in words_df.columns:
            print("YELL DIAG: no alpha_ratio column at all — these words were "
                  "extracted before spectral tilt was added. Re-run the feature extraction.")
...
def _(
    ...
    # recover the between-segment dynamics the per-segment z-scoring
...
def _(
    ...
    # emotion-driven caps + exclamation marks, layered on top of
    ...
    if EXCLAIM_ENABLE:
        styled_word_df = apply_emotion_shout(
            ...
            n_classes=CLF_N_CLASSES,          # chance-relative floors
            p_floor_mult=SHOUT_P_FLOOR_MULT,
            p_second_mult=SHOUT_P_SECOND_MULT,
            primary_emotions=SHOUT_PRIMARY_EMOTIONS,   # strict tiers
    ...
    # MERGE FIRST, SPLIT SECOND. merge_rapid_segments joins captions
    ...
    # silence -> ellipsis, and past a longer threshold -> a new
...
def _(PAUSE_HOLD, PAUSE_HOLD_THRESH, pd, styled_word_df):
    ...
    def styling_coverage(styled_df, label="clip"):
        ...
        _stats = {
            ...
            # caption LINES, not just words. The BreakingBad failure
...
def _(mo):
    ...
    **Read the clamp in `build_ass_events` before touching timing.**
...
def _(
    ...
    def build_ass_events(rows, width, height, tags_fn, layout_mode,
        ...
        for si, sid in enumerate(seg_ids):
            ...
            # reading rate (audit Part 8 item 3): when the natural
            ...
            # a line that was cut at a long pause holds until the line
            ...
            if "line_hold_until" in seg.columns:
                ...
                if _hold > 0.0:
                    e0 = max(e0, _hold)
            # ---- THE CLAMP (read the note before changing timing) --
            ...
            # a caption too brief to read is a DIFFERENT failure
        ...
        # the report that would have caught BreakingBad(Happy) on
...
def _(
    ...
    def render_budget_video(audio_path, df, emotion, styles, out_dir="outputs",
        ...
        # refuse to measure in one face and render in another
...
def _(
    ...
    # {segment_id: speaker_label} from diarization, or all-None if
    ...
    # min_dwell_s wired through -- the second guard alongside the
...
def _(
    ...
    def render_long_video(audio_path, df, styles, out_dir="outputs",
        ...
        # substitution desyncs measurement from rendering — fail here,
    ...
    def process_any_video(video_path, out_tag=None, use_bg_video=True,
        ...
        align_model, align_meta = whisperx.load_align_model(
            language_code=result["language"], device=device)
        aligned = smart_align(result["segments"], align_model, align_meta, audio, device,
                              max_segment_s=6.0, pad_s=1.0)
        # this function loads a SECOND alignment model on top of the
        ...
        # fail HERE, where the cause is visible, rather than three
        ...
        seg_emotion_df = predict_segment_emotions_v9(
            ...
            normalise_mode=SEGMENT_NORM, norm_min_segments=NORM_MIN_SEGMENTS)
        # same speaker-aware wiring as the single-clip test path.
        _speaker_ids_pav = {_i: _s.get("speaker") for _i, _s in enumerate(seg_list)}
        # min_dwell_s wired through here too, so the demo/production
        ...
        styled_df = assign_styles(
            ...
            channel_mode=CHANNEL_MODE,
            emotion_floor_bonus=EMOTION_FLOOR_BONUS)
        # same emotion-driven caps + exclamation marks as the
        ...
        if EXCLAIM_ENABLE:
            styled_df = apply_emotion_shout(
                ...
                max_frac=SHOUT_MAX_FRAC,
                yell_emotion_gate=YELL_EMOTION_GATE)
        # merge first (join captions too brief to read), split
        ...
        styled_df = merge_rapid_segments(
            ...
            max_gap_s=MERGE_MAX_GAP_S, max_chars=MERGE_MAX_CHARS,
            min_line_s=MERGE_MIN_LINE_S)
        # silence -> ellipsis / new subtitle line. Last writer of
...
def _(
    ...
    #
    # TEST_CLIP_ENABLE. This cell is the single most expensive thing
...
def _(mo):
    mo.md(r"""
    ## Part F: Evaluation infrastructure — stimulus screening
...
def _(EMOTION_STYLES, PAUSE_DOTS, PAUSE_DOTS_SKIP_ON_BANG, add_pause_ellipsis):
    ...
    def flatten_to_plain(styled_df, base_font, spec=PLAIN_SPEC):
        ...
        if "word" in out.columns:
            out["word_display"] = out["word"].astype(str)
        # ...except the pause ellipsis, which is put back. The dots
    ...
    return PLAIN_RENDER, flatten_to_plain
        ...
        for _i, _vid in enumerate(_vids, 1):
            ...
            try:
                ...
                _rows.append({
                    ...
                    # caption LINES after merge+split, which is what a
...
def _(mo):
    mo.md(r"""
    run_evaluation_batch(only=["BreakingBad_Happy"], skip_existing=False)
    """)
    return
...
@app.cell
def _():
    import importlib.metadata as verdiag_meta

    for verdiag_pkg in ("whisperx", "faster-whisper", "ctranslate2", "torch", "torchaudio"):
        try:
            print(f"{verdiag_pkg}: {verdiag_meta.version(verdiag_pkg)}")
        except verdiag_meta.PackageNotFoundError:
            print(f"{verdiag_pkg}: not found under that name")
    return
...
@app.cell
def _(Path, os, plt, re, subprocess, whisperx):
    specdiag3_ass_matches = (list(Path("evaluationoutput/ass").glob("*BreakingBad*ass"))
                             if Path("evaluationoutput/ass").exists() else [])
    specdiag3_video_matches = (list(Path("evaluationoutput/video").glob("*BreakingBad*mp4"))
                               if Path("evaluationoutput/video").exists() else [])

    print("found .ass file(s):", specdiag3_ass_matches)
    print("found video file(s):", specdiag3_video_matches)

    if not specdiag3_ass_matches or not specdiag3_video_matches:
        print("one of them wasn't found -- list the folders manually:")
        print(os.listdir("evaluationoutput/ass") if os.path.exists("evaluationoutput/ass") else "no ass dir")
        print(os.listdir("evaluationoutput/video") if os.path.exists("evaluationoutput/video") else "no video dir")
    else:
        specdiag3_video_path = str(specdiag3_video_matches[0])
        specdiag3_ass_path = str(specdiag3_ass_matches[0])

        specdiag3_dur_out = subprocess.run(
            ["ffprobe", "-v", "error", "-show_entries", "format=duration",
             "-of", "default=nw=1:nk=1", specdiag3_video_path],
            capture_output=True, text=True).stdout.strip()
        print("rendered video duration (ffprobe):", specdiag3_dur_out)

        specdiag3_audio_arr = whisperx.load_audio(specdiag3_video_path)
        specdiag3_full_dur = len(specdiag3_audio_arr) / 16000
        print("rendered video's own audio duration (as whisperx reads it):", specdiag3_full_dur)

        def specdiag3_parse_t(s):
            h, m, sec = s.split(":")
            return int(h) * 3600 + int(m) * 60 + float(sec)

        specdiag3_events = []
        for _line in Path(specdiag3_ass_path).read_text(errors="ignore").splitlines():
            if not _line.startswith("Dialogue:"):
                continue
            _fields = _line[len("Dialogue:"):].split(",", 9)
            if len(_fields) < 10:
                continue
            _start = specdiag3_parse_t(_fields[1].strip())
            _text = re.sub(r"\{[^}]*\}", "", _fields[9]).strip()
            specdiag3_events.append((_start, _text))
        print(f"parsed {len(specdiag3_events)} caption event(s) from {specdiag3_ass_path}")

        specdiag3_fig, specdiag3_ax = plt.subplots(figsize=(22, 6))
        specdiag3_ax.specgram(specdiag3_audio_arr, Fs=16000, NFFT=1024, noverlap=512, cmap="magma")
        for _start, _text in specdiag3_events:
            specdiag3_ax.axvline(_start, color="cyan", alpha=0.35, linewidth=0.8)
            if _text:
                specdiag3_ax.text(_start, specdiag3_ax.get_ylim()[1] * 0.95, _text[:20],
                                  rotation=90, fontsize=5, color="white", va="top")
        if specdiag3_events:
            specdiag3_last = max(e[0] for e in specdiag3_events)
            specdiag3_ax.axvline(specdiag3_last, color="red", linewidth=2,
                                 label=f"last caption starts: {specdiag3_last:.1f}s")
            specdiag3_ax.legend(loc="upper right")
        specdiag3_ax.set_xlim(0, specdiag3_full_dur)
        specdiag3_ax.set_xlabel("time (s)")
        specdiag3_ax.set_ylabel("frequency (Hz)")
        specdiag3_ax.set_title(f"rendered BreakingBad(Happy) -- audio is {specdiag3_full_dur:.1f}s wide")
        specdiag3_fig.tight_layout()
        specdiag3_fig.savefig("outputs/breakingbad_align_check3.png", dpi=150)
        print("saved outputs/breakingbad_align_check3.png")
        specdiag3_fig
    return
...
def _(
    ...
    verifyfix_wav_matches = list(Path("outputs/audio").glob("*BreakingBad*wav"))
    verifyfix_audio = whisperx.load_audio(str(verifyfix_wav_matches[0]))

    verifyfix_raw = asr_model.transcribe(verifyfix_audio, batch_size=ASR_BATCH_SIZE,
                                         chunk_size=ASR_CHUNK_SIZE)

    verifyfix_align_model, verifyfix_align_meta = whisperx.load_align_model(
        language_code=verifyfix_raw["language"], device=device)

    verifyfix_segments = presplit_long_segments(verifyfix_raw["segments"], max_segment_s=6.0, pad_s=1.0)
    verifyfix_segments, verifyfix_queue = sanitize_for_alignment(verifyfix_segments)
    verifyfix_aligned = whisperx.align(verifyfix_segments, verifyfix_align_model,
                                       verifyfix_align_meta, verifyfix_audio, device,
                                       return_char_alignments=False)
    verifyfix_aligned = restore_after_alignment(verifyfix_aligned, verifyfix_queue)

    print(f"ALIGNED (presplit + sanitize): {len(verifyfix_aligned['word_segments'])} word(s)")
    for w in verifyfix_aligned["word_segments"]:
        print(f"  {w.get('start')}-{w.get('end')}: {w['word']!r}")
    return
