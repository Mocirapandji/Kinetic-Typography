...
def _(mo):
    mo.md("""
    # V0.4: evidence-based palette + one variable per channel

    V0.3's renders looked busy and I could not say why. Working it out is what
    this version is: **all four visual channels were encoding intensity at the
    same time**, so a loud word arrived bigger, bolder, darker, more saturated
    AND animated. Five cues for one variable reads as noise, not emphasis.

    Two changes follow from that.

    **The colours stop being mine.** V0.3 reused the hues from the RF/MLP demo
    videos, which I picked because they looked right. They are now taken from the
    Jonauskaite 37-nation colour-emotion survey, assigned by maximum-weight
    matching, with saturation and value set from Valdez & Mehrabian's
    arousal regressions. Per-emotion fonts come in alongside them.

    **Each channel gets exactly one job.**
    ...
    The alpha reveal from V0.3 becomes a **wipe**: instead of snapping on its
    cue, each word blooms from dim to opaque across its own spoken duration, so
    the opacity front crawls along the line with the voice. Motion is now gated
    to the single top-scoring word of a sentence, and often fires on nothing at
    all.
    """)
    return
...
def _(os):
    ...
    out_tag = "v0_4_" + DATASET + "_" + audio_file.split("/")[-1].replace(".wav", "")
...
def _():
    ...
    #   OPACITY                    -> TIME. The wipe, nothing else.
    #   MOTION                     -> THE EXCEPTION. One word per sentence, max.

    # ----- TYPOGRAPHY channel: the sole carrier of intensity -----
    # Range widened from 34..60, because size no longer competes with colour to
    # say "this word matters" -- it is now the only thing saying it.
    BASE_FONT_SIZE = 32
    ...
    REVEAL_MODE = "wipe"       # "wipe" | "snap" (V0.3 behaviour) | "none"
    ...
    # hue      : Jonauskaite 37-nation survey (n=8,615), maximum-weight
    #            assignment rather than picked by eye.
    # sat/value: Valdez & Mehrabian (1994) PAD regressions inverted per emotion,
    #            then compressed into a legible band and separated for
    #            discriminability (sad vs fearful, disgust vs surprised).
    # font     : per-emotion family, chosen for what is installed on a typical
    #            Linux image. Categorical, like colour.
    # italic   : also categorical -- disgust is ALWAYS italic. It identifies the
    #            emotion, it does not signal emphasis.
    EMOTION_STYLES = {
...
def _(EMOTION_STYLES, subprocess):
    ...
    # name never crashes -- it just quietly ignores the font choice. This reports
    # which mapped fonts actually resolve on THIS machine.
...
def _(np):
    ...
    def compute_salience(words_df, weights, zero_missing=frozenset(), eps=1e-9):
        ...
        A feature with near-zero variance inside a segment carries no emphasis
        information there, so its z stays 0 rather than dividing by ~0. Features
        in `zero_missing` treat a raw 0 as "no data" and get z = 0.
        """
        df = words_df.copy()
...
def _(
    ...
    # 0 at fair share, 1 at FULL_DRAMA_RATIO x fair share, restrained by
...
def _(mo):
    mo.md("""
    ## Part D: Style mapping — one colour per clip, size per word

    Hue, saturation and value come from the clip's emotion and are identical for
    every word in it. Size and weight are the only things that vary word to word.
    Motion is decided here too, but only as a flag: which single word in each
    sentence, if any, is allowed to move.
    """)
    return
...
def _(
    ...
    # CELL 17 — STYLE MAPPING (one variable per channel)
    def emotion_color(emotion, styles):
        """One fixed colour for the whole clip. Colour says WHICH emotion -- it
        does not also say which word matters. That job belongs to size."""
...
def _(mo):
    mo.md("""
    ## Part E: Render — wipe reveal + gated emphasis

    Two mechanics worth knowing:

    - **The wipe** animates each word's alpha over its real spoken interval, so
      the opacity front travels with the voice. A true pixel clipping mask
      ...
      fragility.
    - **Emphasis moves** fire only above EMPHASIS_TRIGGER, and only on the
      sentence's winner. The bounce is a `\\fscy`-only pulse, so width never
      changes and the line cannot reflow. The angry stretch uses `\\fscx` and IS
      the one deliberate exception to that: the line breathes outward for about
      400 ms on one word. If it reads badly in motion, set
      EMPHASIS_ANIM["angry"] = "bounce".
    """)
    return
...
def _(
    ...
    def render_budget_video(audio_path, df, emotion, styles, out_dir="outputs",
        ...
        def word_tags(rw, line_start):
            # COLOUR + TYPOGRAPHY: colour is the clip's emotion; size and weight
            # carry intensity. Set once, never animated.
            ...
            # opaque over its own spoken interval, so the front moves with the
            ...
            # MOTION: only the sentence's winner, only if it cleared the trigger
            # (`moves` was decided in CELL 17). Usually 0-1 per line.
        ...
        if caption_mode == "word":
            ...
            for si, sid in enumerate(seg_ids):
                ...
                if si < len(seg_ids) - 1:
                    e0 = min(e0, seg_starts[seg_ids[si + 1]])   # never overlap the next line
                e0 = max(e0, s0 + 0.10)
...
def _(mo):
    ...
    word arrived bigger, bolder, darker, more saturated AND bouncing. Five cues
    for one variable reads as noise, not emphasis, and it is the single reason
    the earlier renders looked bad. Each channel now carries exactly one thing.
    Font family and italics are *categorical* markers, like colour: they
    identify the emotion, they never signal emphasis.

    ### Individual decisions

    1. **Hue palette from survey data.** Jonauskaite et al. 37-nation survey
       ...
       Sadness takes blue: the survey ranks it third, but it is rank 1 across
       23 studies in the 128-year systematic review, and it keeps sad and
       fearful discriminable in the mute test. Surprised takes orange, resting
       on review counts alone because surprise is absent from the Geneva
       Emotion Wheel — the weakest anchor in the palette.
    2. **Colour is constant across a clip.** Valdez & Mehrabian measured how *a
       colour* makes a viewer feel, which maps to the *emotion*, not to
       ...
       and separated for discriminability. Using the same equations as a
       per-word ramp would be a misapplication of the source.
    3. **Legibility constraints are documented, not hidden.** Fear's
       ...
       video. A white caption plate was trialled (BorderStyle=4) and rejected as
    ...
    4. **Per-emotion fonts, verified rather than assumed.** libass falls back
       silently on a missing family, so a wrong font name never errors, it just
       ignores the choice. CELL 12 shells out to `fc-list` and reports what
       actually resolves on this machine.
    5. **Typography is the sole intensity channel**, so its range widened to
       32–64 px once it stopped competing with colour to say the same thing.
    6. **Opacity encodes time and nothing else.** Every word rests
       half-transparent and blooms to fully opaque over its own spoken interval,
       so the front travels across the line with the voice. A literal pixel
       clipping mask (animated \\clip) would need per-word x-positions and font
       metrics; word-granular alpha gives the same read without that fragility.
    7. **The trailing-voice fade was built, then removed.** Quiet words settling
       short of opacity was expressive, but it put a second variable on the
       opacity channel, so an unspoken word and a quiet word looked alike —
       exactly the confusion this redesign exists to eliminate. Loudness already
       feeds salience, so quiet words are handled through the budget →
       typography path instead. Worth revisiting on a channel that is genuinely
       free.
    8. **Motion is rare by construction**: only the single top-scoring word in a
       sentence animates, and only if it also clears intensity 0.75. Often zero
       words per line. Motion is punctuation, not decoration.
    9. **Everything upstream is untouched**: classifier, budget and prosody
       extraction are unchanged. Every version since V0.2 has only re-mapped
       emotion and intensity onto visuals.

    ### Known weakness

    Motion currently fires on an intensity threshold, which makes it the
    typography channel again, just jumpier. It is not carrying a variable of its
    own the way the other three are. It should encode something size cannot —
    pitch direction is the obvious candidate, since it varies word to word
    inside a single emotion and nothing else in the system reports it.
    """)
    return
