...
def _(mo):
    mo.md(r"""
    # V0.8-9: three fixes, then the audit

    Two rounds of work, folded together.

    **First, three things that were plainly broken.**

    *The whole caption jumped.* Motion was `\fscy` applied to one word inside a
    single `Dialogue` event holding the entire sentence. libass sizes a line box
    from its tallest run, so growing one word grew the box and shifted every
    other word's baseline. Attenuating the swell would have made the problem
    smaller rather than fixing it. Each word is now measured with the real font
    file and emitted as its own `\pos`'d event, so a word can swell, tilt or
    wobble without touching its neighbours.

    *The wrong words were emphasised.* Salience was `w * |z|`. The absolute value
    meant a word that was unusually *quiet*, *short* and *low* scored exactly
    like one that was unusually loud, long and high — and since unstressed
    function words are reliably quiet and low, the metric was actively selecting
    for them. That is why "OR", "the" and "a" kept blowing up. Emphasis is now
    positive-only and signed, with robust statistics and a word-class prior whose
    damping is *conditional*: a function word carrying genuine contrastive stress
    ("this OR that") clears the override threshold and recovers full weight.

    *The transcript was worse than it needed to be.* Whisper ran at "base" with
    no language hint and no decoding guards, and every prosodic measurement sits
    on top of those word boundaries. Model size dominates word error rate, so it
    now defaults to large-v3 on a GPU, with the language pinned and guards
    against the known repetition-loop failure.

    **Then, a section-by-section revision against a literature audit.** The
    palette is now chosen by perceptual measurement — a CIEDE2000 search under
    colour-vision-deficiency simulation — rather than by eye. Fear's arousal
    encoding turned out to be inverted in both colour and tempo, and is
    corrected. The confidence curve no longer renders a 0.43 posterior as
    near-certainty. The one-variable-per-channel rule becomes an explicit switch
    with the evidence for each position recorded. The swell becomes a proper
    volume-conserving squash and stretch rather than a scale change. New cells
    re-derive the palette distances, report styling coverage, and screen stimuli
    for lexical emotion content, all on every run rather than once.
    """)
    return
...
def _(os, torch):
    ...
    # torch now comes from CELL 0 (marimo forbids defining a name in two
    ...
    out_tag = "v0_8_9_" + DATASET + "_" + audio_file.split("/")[-1].replace(".wav", "")
...
def _(gc, torch):
    ...
    def cuda_report(tag=""):
        ...
        print(f"VRAM {tag}: driver-free {_free/2**30:5.2f} / {_total/2**30:5.2f} GiB"
              f" | torch allocated {_alloc/2**30:5.2f}, reserved {_resv/2**30:5.2f} GiB"
...
def _(ADJ_ADV_TIER, CONTENT_MOD_PRIOR, np, re):
    ...
    # ------------------------------------------------------------------
    # New: suffix heuristic for adjective/adverb detection (audit 5.2).
    ...
    def classify_words(word_series, adj_adv_tier=ADJ_ADV_TIER,
        ...
        # New: promote suffix-marked adjectives/adverbs to their own tier.
...
def _():
    ...
    # with the HIGH-arousal states; V0.7 had fearful in "soft" at 1.50x, the
    ...
    # ----- New: CHANNEL RULE, made explicit (audit Part 1) --------------
    ...
    # ----- New: CONFIDENCE CURVE (audit 5.4) ----------------------------
    ...
    # ----- New: SQUASH AND STRETCH (audit 4.3) --------------------------
    ...
    # ----- New: READING RATE (audit Part 8 item 3) ----------------------
    ...
    # ----- New: CONTRAST OVER ARBITRARY VIDEO (audit Part 8 item 2) -----
    ...
    # ----- New: FONT SUBSTITUTION (audit Part 8 item 8) -----------------
    ...
    # ----- New: ADJ/ADV PRIOR TIER (audit 5.2) — default OFF ------------
    ...
    # ----- New: EmoLex lexicon for stimulus screening (audit 7.1) -------
    ...
    # ----- COLOUR channel base per emotion (new palette) ----------------
    ...
    #     fearful=violet 252deg kept BUT s/v corrected: V0.7's (0.28, 0.88)
    ...
    # Worst-case pairwise dE2000, V0.7 -> this version (from cell 12d):
...
def _(
    ...
    # ---------------------------------------------------------------------
    # Re-derives, on every run, the numbers the new palette was chosen by,
    ...
    #     caught V0.7's fear bug (arousal = -0.31*V + 0.60*S applied to the
    ...
    assert _arousal["fearful"] > _arousal["sad"], (
        ...
        "V0.7 bug (fear at -0.10 vs sad +0.08) made permanent")
    ...
    def attach_motion(words_df, motion_source, motion_min_intensity,
        ...
        def _gesture(r):
            ...
            if float(r["intensity"]) < motion_min_intensity:
                return "none"
            # Note: no gesture without a trustworthy pitch contour. V0.7 let a
...
def _(font_vmetrics, pause_gap, space_width, text_width):
    ...
    # a squashed word will occupy. V0.7 had 0.6/0.5/0.4 as magic numbers in
    ...
    def layout_segment(seg_rows, width, height, margin_x, margin_v,
        ...
        for _, rw in seg_rows.iterrows():
            ...
            # New: horizontal headroom for the squash (see swell_headroom_x)
...
def _(
    ...
    if USE_CONFIDENCE_SCALING:
        # New: calibrated curve from cell 12b (0.43 -> ~0.60, not ~0.90)
...
def _(
    ...
    def render_budget_video(audio_path, df, emotion, styles, out_dir="outputs",
        ...
        # New: refuse to measure in one face and render in another
...
def _(
    ...
    def render_long_video(audio_path, df, styles, out_dir="outputs",
        ...
        # New: substitution desyncs measurement from rendering — fail here,
...
def _(mo):
    ...
    ### The three fixes
    1. **Motion is isolated by absolute layout, not by attenuation.** V0.7
        ...
        smaller, not fixed it. This version measures each word with the font file
    ...
    2. **Emphasis is directional.** V0.7 scored `w * |z|`, which treated a word
        ...
        this version clips the emphasis features at zero so only above-average loudness,
        pitch and lengthening earn points. HNR stays two-sided, because creak
        and breathiness are both marked voice qualities.

    3. **Duration is a residual, not a raw value.** Raw duration conflates
        "this word has four syllables" with "the speaker leaned on this word".
        this version fits duration against syllable count across the clip and scores the
    ...
    4. **Pitch is tracked once, with a speaker-adapted range.** V0.7 re-ran
        ...
        single halved frame into an enormous apparent range. This version tracks the
    ...
    5. **A word-class prior, with a prosodic override.** Determiners,
    ...
    6. **An emphasis quota.** Even with correct scoring, a line where a third
    ...
    7. **ASR quality is a model-size problem first.** V0.7 ran whisper "base"
        with no language hint. Size dominates WER, so this version defaults to large-v3
    ...
    ### The audit revision
    8. **Palette hues are chosen by measurement, not by eye.** A CIEDE2000
        ...
        hue-crowding problem and the CVD problem were the same problem. the new
    ...
    9. **Fearful's arousal inversion is fixed in both channels it appeared
        in.** V0.7 encoded fear at saturation 0.28 / value 0.88, which the
        ...
        high-arousal states). Now (0.70, 0.78) → +0.18, above sad, and a new
    ...
    10. **The colour science self-checks or refuses to run.** ΔE2000 follows
    ...
    11. **The channel rule is resolved as an explicit, evidenced switch.**
    ...
    12. **The confidence curve stops overstating the classifier.** The legacy
    ...
    13. **The swell is volume-conserving squash-and-stretch.** Every `\fscy`
    ...
    14. **Reading rate is enforced, bounded, and reported.** Broadcast caption
        practice works to a characters-per-second ceiling; this version extends a
    ...
    15. **Legibility parameters are visible, and substitution is a failure.**
    ...
    16. **The adjective/adverb tier ships off.** EmoLex's POS breakdown
    ...
    17. **The saturation floor is a named trade-off.** At intensity 0 every
