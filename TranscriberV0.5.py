...
def _():
    ...
    import json          # ffprobe output parsing, for the real-video render
...
def _(mo):
    mo.md("""
    # V0.5: pitch-driven motion + any video in

    Two things, and the first one is the weakness V0.4 ended on.

    **Motion gets its own variable.** V0.4 fired motion on an intensity
    threshold, which made it the typography channel again, just jumpier. It now
    follows the **f0 slope within each word** — rising pitch lifts, falling pitch
    drops, an unsteady contour wobbles. That is information no other channel
    carries, and unlike intensity it varies word to word *inside a single
    emotion*. The four channels are finally independent:

    | Channel | Encodes | Varies |
    |---|---|---|
    | Colour (hue, saturation, value) | which emotion | per segment |
    | Typography (size, weight) | word intensity | per word |
    | Opacity | time — the wipe | per word, continuously |
    | Motion | pitch direction | per word |

    **Emotion stops being a property of the file.** Everything so far assumed
    one file = one sentence = one emotion, which is true of RAVDESS and of
    IEMOCAP's per-utterance wav cuts. Handing the pipeline a real video — a full
    conversation, or any clip dropped in — breaks that. The same classifier and
    the same 14 features now run once per WhisperX segment on that segment's own
    audio, so colour, font and confidence change turn by turn. The renderer can
    also burn onto the source footage instead of a black screen.

    Two paths coexist below. CELLS 13/17/18 are the original single-clip demo,
    kept so the RAVDESS/IEMOCAP runs still reproduce. CELLS 19-24 are the
    per-segment path that takes any video.
    """)
    return
...
def _(os):
    ...
    out_tag = "v0_5_" + DATASET + "_" + audio_file.split("/")[-1].replace(".wav", "")
...
def _(call, np, parselmouth):
    # CELL 4 — CLIP-LEVEL FEATURE EXTRACTOR
    # Split in two so the same 14 features can be computed on an in-memory
    # Sound slice (a segment) as well as a file. extract_clip_features(path)
    # behaves exactly as before, so CELLS 5/6/13 do not change.
...
def _(RandomForestClassifier, clip_df):
    ...
    # Trains on ALL cached clips; the honest speaker-independent number (46.8%
    # vs 14.3% chance) lives in train_emotion.py and is not re-run here.
...
def _(audio_file, compute_type, device, whisperx):
    ...
    # asr_model is returned as well now, because CELL 22 reuses it rather than
...
def _(call, np, parselmouth, pd):
    ...
    # Two changes here.
    #
    # f0_slope is new: the slope of the pitch contour across the word, in
    # semitones per second. Semitones rather than Hz so the value is
    # perceptually scaled and comparable between a low and a high voice.
    #
    # Every Praat call is now individually guarded. to_pitch() and
    # to_harmonicity() need a minimum window length (roughly 0.04-0.06s at
    # default settings) and RAISE rather than returning "no pitch" when a word
    # is shorter than that. Curated single-sentence clips never produced a word
    # that short; a raw conversation's fast function words, interjections and
    # alignment artifacts do, and it was crashing the whole file over one bad
    # word. A too-short word now gets the same zero fallback that "unvoiced"
    # already used.
    def extract_word_features(audio_path, word_segments):
        ...
        for i, w in enumerate(word_segments):
            ...
            # falls back to 0.0 when f0v is empty or too short to fit a line
            if len(f0v) >= 3:
...
def _():
    ...
    # Still gated by salience, so unstressed function words with steep slopes
    # stay still: a word must be prosodically prominent AND its pitch must be
    # doing something before it moves at all.
    ...
    #   never reflow and the baseline can never break.
...
def _(mo):
    mo.md("""
    ### Step C1: Clip-level emotion prediction (the single-clip path)
    """)
    return
...
def _(
    ...
    # CELL 17 — STYLE MAPPING, single-clip path
    ...
    def assign_styles_v6(words_df, emotion, styles,
        ...
        # actually doing something. Unstressed function words with steep slopes
        # stay still; flat prominent words stay still too.
        def _gesture(r):
...
def _(mo):
    ...
    - **Every word re-declares every tag it uses.** .ass override tags persist
      ...
      reset those tags, or the effect leaks down the sentence. That is why
      `\\fscx100\\fscy100\\frz0` appears in every word's tag block.
    - **`\\fscy` swell is the default motion**, because it cannot break the
      line: width never changes, so no reflow, and the baseline stays flat.
      `\\frz` tilt is more literal but rotation propagates aggressively along an
      .ass line and visibly breaks the baseline if any word forgets to reset it.
      Both are implemented; MOTION_STYLE picks.
    - **There is no per-word position tag in .ass.** `\\pos` and `\\move` are
      line-level, like `\\fad`. So a "lift" is a swell or a tilt, never a true
      translation. That is a format constraint, not a design choice.
    """)
    return
...
def _(
    ...
    def render_budget_video(audio_path, df, emotion, styles, out_dir="outputs",
        ...
        def word_tags(rw, line_start):
            # COLOUR + TYPOGRAPHY, set once, never animated. \fscx100\fscy100
            # \frz0 are RESETS: .ass tags persist along a line, so without them
            # a previous word's swell or tilt would leak into this one.
            ...
            # the line with the voice. Every word ends at alpha 00.
...
def _(mo):
    ...
    CELL 13 predicts one emotion for the whole file, which is right for a
    single-sentence clip and wrong for a conversation. From here on the same
    classifier runs per segment, and the renderer can burn onto real footage.
    """)
    return
...
def _(emotion_color, extract_clip_features_from_sound, np, parselmouth, pd):
    # CELL 19 — PER-SEGMENT EMOTION
    # Same classifier (clf_full), same 14 features. The only thing that changed
    # is granularity: once per WhisperX segment, on that segment's own audio
    # slice, instead of once for the whole file.
    def predict_segment_emotions(audio_path, segments, clf, feature_cols):
        ...
        for sid, seg in enumerate(segments):
            ...
            try:
                ...
                # segment too short or silent for Praat -- fall back to neutral
    ...
    def assign_styles_v8(words_df, seg_emotion_df, styles, base_font, font_swing, bold_thresh):
        ...
        # intensity_raw is the budget share only. The single clip-wide
        # conf_scale multiply is replaced with each word's OWN segment's
        # confidence, so a confidently-classified sentence styles at full
        # strength and a shaky one is damped, sentence by sentence.
...
def _(json, os, subprocess):
    ...
    def render_long_video(audio_path, df, styles, out_dir="outputs",
        ...
        2. bg_video_path: if given, THAT video is the picture. If it already has
           ...
           muxed in. bg_video_path=None falls back to the black screen exactly
           as before, so the single-clip demo still runs unchanged.
        """
        os.makedirs(f"{out_dir}/ass", exist_ok=True)
        os.makedirs(f"{out_dir}/video", exist_ok=True)
        ...
        if bg_video_path:
            ...
            # speaker) and should not get truncated.
            if bg_has_audio:
...
def _(
    ...
    # Needs CELLS 0-13 already run once, so clf_full, asr_model and
    # clip_feature_cols exist.
...
def _(iemocap_dir, os, process_any_video):
    ...
    # RAVDESS-trained classifier is now labelling natural conversational turns
    # it never saw in training, so the acted-vs-spontaneous gap is visible in
    # the render itself rather than only in a held-out accuracy number.
    def find_iemocap_dialog_video(utt_id, iemocap_dir):
        ...
        `dialog` derivation CELL 1 already uses.

        ASSUMPTION to verify: IEMOCAP_full_release still has dialog/avi/. It
        ships in the standard release; if this copy doesn't have it, the
        function returns None and the fallback is use_bg_video=False.
        """
    ...
    _demo_utt = "Ses01F_impro01_F012"     # any utterance id from the dialog wanted
...
def _(mo):
    ...
    Earlier versions had all four channels encoding intensity at once — a loud
    word arrived bigger, bolder, darker, more saturated AND animated. Five cues
    for one variable reads as noise, not emphasis. Each channel now carries
    exactly one thing, and as of this version motion finally carries a variable
    of its own rather than a second copy of intensity. Font family and italics
    stay *categorical* markers, like colour: they identify the emotion, they
    never signal emphasis.

    ### Motion

    1. **Motion follows pitch, not intensity.** V0.4 gated motion on an
       intensity threshold, which made it the typography channel again. It now
       reads the f0 slope within each word: rising lifts, falling drops, a wide
       pitch range wobbles. Pitch direction is information no other channel
       carries, and unlike intensity it varies word to word inside a single
       emotion.
    2. **f0 slope is measured in semitones per second**, fitted over the voiced
       frames of the word, so the value is perceptually scaled and comparable
       between a low and a high voice rather than being a raw Hz difference.
    3. **The gesture is still gated by salience.** A word must be prosodically
       prominent AND have pitch that is doing something. Unstressed function
       words with steep slopes stay still, and so do flat prominent words.
    4. **`\\fscy` swell is the default and `\\frz` tilt is the option.** Swell
       cannot break the line, because width never changes so nothing reflows and
       the baseline stays flat. Rotation is more literal but propagates along an
       .ass line, so every word has to reset it or the tilt leaks into the rest
       of the sentence. Both are implemented and MOTION_STYLE picks.
    5. **There is no per-word position tag in .ass.** `\\pos` and `\\move` are
       line-level, so a "lift" is a swell or a tilt, never a true translation.
       A format constraint, not a design choice.

    ### Any video in

    6. **Emotion is predicted per segment, not per clip.** One emotion per file
       is correct for RAVDESS and wrong for a conversation, where the emotion
       changes turn to turn. `predict_segment_emotions` reuses the exact same
       classifier and the same 14 features — nothing about the model changed,
       only its granularity.
    7. **Colour, font, italic and anim now vary by segment.**
       `assign_styles_v8` looks each word's emotion up from its `segment_id`,
       replacing the single clip-wide `emotion` argument. `attach_motion` needed
       no change at all, which is a nice confirmation of decision 1: pitch
       direction really is independent of which emotion won.
    8. **Confidence is per segment too.** It used to be one number for the whole
       file. A confidently-classified sentence now styles at full strength and a
       shaky one is damped, sentence by sentence rather than for the whole
       conversation at once.
    9. **Rendering can burn onto real footage.** `render_long_video` takes an
       optional `bg_video_path`. If given, that video is the picture: its own
       audio is kept if it has one, otherwise the transcribed audio is muxed in.
       Output duration comes from the video file via ffprobe, not from the last
       transcribed word plus padding, so real footage never gets truncated.
       `bg_video_path=None` reproduces the black-screen behaviour exactly.
    10. **The existing dataset can supply that video.**
        `find_iemocap_dialog_video` derives IEMOCAP's full dialog recording path
        from an utterance id, using the same derivation CELL 1 already had. It
        also means the RAVDESS-trained classifier is being asked to label
        natural conversational turns it never saw in training — the
        acted-versus-spontaneous gap, now visible in the render.
    11. **Per-word extraction was assuming every word is long enough.**
        `to_pitch()` and `to_harmonicity()` need roughly 0.04-0.06s at default
        settings and raise rather than returning "no pitch". Curated
        single-sentence clips never produced a word that short; a raw
        conversation's fast function words, interjections and alignment
        artifacts do. Each call is now guarded per word and falls back to the
        same zero values already used for unvoiced, instead of crashing the
        whole file over one bad word.
    """)
    return
