...
def _(mo):
    mo.md(r"""
    # V1.1: emotion as a sequence, not a series of guesses

    Everything so far classified each segment on its own. That quietly assumed
    a speaker's emotion is redrawn from scratch every sentence — but five angry
    sentences are five samples of one state, not five unrelated draws. With a
    classifier at this accuracy, independent argmax flips the caption's colour
    on posterior differences (0.34 / 0.29 / 0.36) far smaller than the model's
    own error, so the palette strobes while the delivery has not changed.

    **A first-order HMM over the segment sequence.** The emissions are the
    posteriors already computed, the transition matrix favours staying put, and
    Viterbi decoding picks the most likely *path*, so a switch has to be paid
    for in likelihood. Two properties make this the right tool rather than a
    blur: it is confidence-weighted for free, since a sharp distribution
    overrules its neighbours and a flat one is carried by them, and it can only
    ever choose labels the classifier actually proposed, so no colour can appear
    that no segment supported. The self-transition bias was swept on synthetic
    cases rather than guessed, and both the usable window and the value adopted
    are recorded next to the parameter.

    **A minimum on-screen duration, because the bias alone cannot see run
    length.** `self_bias` is a per-step cost: it penalises switching at the
    moment it happens, but it has no view of how long the run that follows
    actually lasts. A single noisy segment can still win a two-segment
    tug-of-war and flash on screen for under a second. Duration is a
    complementary signal — a colour state that lasted a fraction of a second is
    almost certainly classifier noise regardless of what the bias decided when
    it let the switch through — so a second pass merges runs shorter than
    `MIN_DWELL_S` into whichever neighbour has stronger emission support.

    **Speaker diarization, because the persistence assumption breaks across a
    speaker change.** The smoother cannot tell one noisy segment apart from a
    different person starting to talk, and on a fast multi-speaker argument that
    works actively against you: on one test clip it overwrote every segment
    where the raw classifier had picked anger as an outright winner. Speaker
    labels are now attached to segments, and at a speaker boundary the
    transition matrix is swapped for a uniform one — a new speaker's emotion is
    not evidence about the previous speaker's. The dwell pass is speaker-aware
    for the same reason: a short run is never absorbed into a neighbour
    belonging to someone else.

    **The smoother nonetheless ships off, and that is the honest result.** The
    original sweep was run against a different model with sharper
    distributions, so pretending those numbers still apply after pairing it with
    diarization would be the same mistake twice. It stays off until the pairing
    is swept properly.

    Also here: the whole notebook was compacted from 52 cells to 32, verified
    content-preserving rather than assumed so, and a second emotion-driven
    shout trigger was added alongside the acoustic one — with a note saying
    plainly that it departs from the acoustic detector's own design principle.
    """)
    return
...
def _(
    ...
    def enforce_min_dwell(path, seg_df, min_dwell_s, S, speaker_ids=None):
        ...
        NEW: `speaker_ids`, if given (a per-segment array aligned with
    ...
    def smooth_segment_emotions(seg_df, classes, mode="viterbi",
        ...
        * It cannot invent a label. Viterbi picks from what the classifier
        ...
        NEW: after the path is decoded, enforce_min_dwell runs as a second,
        ...
        NEW: `speaker_ids`, a dict {segment_id: speaker_label} from
        ...
        # NEW: resolve speaker_ids (a {segment_id: label} dict) against
        ...
        if mode == "ema":
            ...
            # NEW: the reset matrix used at a speaker boundary instead --
        ...
        # NEW: duration-based cleanup pass, independent of self_bias.
        # NEW: speaker-aware -- a short run is never merged across a
    ...
    def predict_segment_emotions_v9(audio_path, segments, clf, feature_cols,
        ...
        for m in meta:
            ...
            # NEW: keep the WHOLE posterior, not just the top two. Smoothing
...
def _(
    ...
    # NEW: hand torch's cache back to the driver BEFORE CTranslate2 asks
...
def _(os):
    ...
    SEGMENT_AROUSAL_FLOOR = 0.45     # 0.0 = V1.0 behaviour (no floor)
    AROUSAL_FEATURES = {"intensity_db": 1.0, "f0_mean": 0.6}
    AROUSAL_SPREAD = 1.5             # z at which the floor is ~76% of max

    # ----- NEW: PER-EMOTION FLOOR BONUS (declare this if you use it) --
    ...
    # ----- NEW: SHOUTING DETECTION AND CASE --------------------------
    ...
    # ----- NEW: TEMPORAL SMOOTHING OF EMOTION -------------------------
    ...
    # NEW: switched off. On fast multi-speaker dialogue (12 Angry Men)
    ...
    # NEW: raised 0.55 -> 0.68. The swept "usable window" above (0.40-0.65)
    ...
    # (per the sweep above), so this version pairs a modest bump here with a
    ...
    # NEW: minimum on-screen duration for a smoothed colour run.
    ...
    # ----- NEW: EMOTION-DRIVEN SHOUT MARKERS ------------------------
    ...
    # acoustic yelling (the pre-existing pre-existing behaviour).
    EXCLAIM_ENABLE = True
    EXCLAIM_EMOTIONS = {"angry", "happy"}   # "mad" and "close to mad / happy"
    ...
    # NEW: tightened 0.20 -> 0.08. Blending between the top-2 emotion
    ...
    BLEND_MARGIN = 0.08
    # NEW: lowered 0.30 -> 0.0. Every word inside an ambiguous segment now
...
def _(colorsys, np, re):
    ...
    def assign_styles(words_df, seg_emotion_df, styles,
        ...
        # NEW: the segment's absolute arousal sets a FLOOR, then the
    ...
    def apply_emotion_shout(styled_df, emotions, blend_margin,
        ...
        """NEW — a SECOND, emotion-driven shout trigger, independent of
        ...
        def _is_shout_emotion(row):
                # 1. Direct match: only trigger if the winning emotion is
                # itself at least 50% confident. On this classifier (7
                # classes, ~60.6% accuracy) that is close to the ceiling of
                # what it typically produces, not a moderate bar -- verified
                # against the 12 Angry Men clip: only 2/19 happy/angry
                # segments ever cleared 0.50, max observed there was 0.537.
                # Raise further (0.70/0.80) only if you want this clause to
                # fire rarely-to-never on this classifier.
                if row["emotion"] in emotions and float(row["p_top"]) >= 0.40:
                    return True
                ...
                # confident candidates were neck-and-neck."
                e2 = row.get("emotion2")
                if (e2 in emotions
                        and (float(row["p_top"]) - float(row["p_second"])) <= blend_margin
                        and float(row["p_second"]) >= 0.30):
                    return True
...
def _(
    ...
    # NEW: recover the between-segment dynamics the per-segment z-scoring
...
def _(
    ...
    # NEW: emotion-driven caps + exclamation marks, layered on top of
...
def _(
    ...
    # NEW: {segment_id: speaker_label} from diarization, or all-None if
    ...
    # NEW: min_dwell_s wired through -- the second guard alongside the
...
def _(
    ...
    def process_any_video(video_path, out_tag=None, use_bg_video=True):
        ...
        aligned = whisperx.align(result["segments"], align_model, align_meta, audio, device,
                                 return_char_alignments=False)
        # NEW: this function loads a SECOND alignment model on top of the
        ...
        seg_emotion_df = predict_segment_emotions_v9(
            ...
            normalise_mode=SEGMENT_NORM, norm_min_segments=NORM_MIN_SEGMENTS)
        # NEW: same speaker-aware wiring as the single-clip test path.
        _speaker_ids_pav = {_i: _s.get("speaker") for _i, _s in enumerate(seg_list)}
        # NEW: min_dwell_s wired through here too, so the demo/production
        ...
        styled_df = assign_styles(
            ...
            channel_mode=CHANNEL_MODE,
            emotion_floor_bonus=EMOTION_FLOOR_BONUS)
        # NEW: same emotion-driven caps + exclamation marks as the
...
def _(mo):
    ...
    1. **A minimum on-screen duration, separate from the switching cost.**
        `self_bias` is a PER-STEP cost: it penalises switching once, at the
        moment it happens, but it cannot see how long the run that follows
        actually lasts. A single noisy segment can still win a two-segment
        tug-of-war against it and flash on screen for under a second. Duration
        is a different and complementary signal — a colour state that only
        lasted a fraction of a second is almost certainly classifier noise
        regardless of what `self_bias` decided when it let the switch through.
        `enforce_min_dwell` scans the decoded path for runs shorter than
        `MIN_DWELL_S` in wall-clock time and merges each into whichever
        neighbouring run has the stronger total emission support, repeating
        until none are left. It cannot invent a label that was never in the
        path, and a run with no neighbour is left alone. So `self_bias`
        controls the per-step cost of switching and `MIN_DWELL_S` controls the
        minimum on-screen duration of whatever the path decided.

    2. **Speaker diarization, because persistence does not hold across a
        speaker change.** The smoother cannot distinguish one noisy segment
        from a different person starting to talk, which on fast multi-speaker
        dialogue is not a small error: on one 51-segment test clip it
        overwrote every segment where the raw classifier had picked anger as
        an outright winner. Speaker labels are now attached to segments, and
        the smoother resets at a boundary — Viterbi swaps the transition
        matrix for a uniform one, EMA restarts from the raw posterior in both
        directions, and the dwell pass refuses to merge a short run into a
        neighbour belonging to someone else. A new speaker's emotion is not
        evidence about the previous speaker's. It fails soft: a missing token
        prints setup guidance and the pipeline continues with all-None speaker
        labels, which reproduces the unaware behaviour exactly.

    3. **The smoother ships off, and that is the honest result.** Having built
        the sequence model and then the two guards for it, the default is still
        `"off"`. The original self-bias sweep was run against a different model
        with sharper distributions, so carrying those numbers forward as if
        they still applied — now that diarization changes what the transitions
        even mean — would be the same mistake the sweep existed to avoid. It
        stays off until the pairing is swept properly. The parameters, the
        sweep results and this reasoning all stay in place so the re-sweep has
        something to start from.

    4. **The whole posterior is kept, not just the top two.** Smoothing needs
        every class probability per segment, not the argmax and its runner-up,
        so the predictor now emits one column per class. It also makes the
        per-segment diagnostic possible: smoothed label against raw argmax,
        with the full distribution beside it, which is the only way to see
        whether the smoother is fixing noise or erasing signal.

    5. **A second, emotion-driven shout trigger — and it contradicts the
        first.** The acoustic detector deliberately reads vocal effort and is
        blind to the classified emotion, because people shout when happy and
        are quietly furious. This new trigger fires on the emotion instead,
        reading the same top-2 blend evidence the colour already uses so there
        is no second threshold to keep in sync. That is a real departure from
        the acoustic detector's stated principle, made because the ask was
        "make anger read as shouted" rather than "measure whether shouting
        occurred". It is worth stating plainly in any write-up, because it
        changes what "capitals mean shouting" is actually keyed on here.
        `EXCLAIM_ENABLE = False` restores pure acoustic yelling.

    6. **The blend was tightened once the whole posterior was visible.** The
        ambiguity margin dropped and the per-word swing went to zero, because
        with the full distribution on screen in the diagnostic it was clear the
        old settings were blending segments the classifier was not actually
        torn about, and varying the blend word by word inside one segment made
        a single sentence read as two.

    7. **Emotion is a sequence, not seven independent decisions.** Classifying
    ...
    8. **Compaction is only safe if it is proved.** 52 cells became 32 by
    ...
    9. **Environment failures should degrade, not halt.** `FONT_STRICT`
    ...
    10. **The salience budget cannot represent a uniformly loud line.**
    ...
    11. **Capitals are a standard, not a styling idea.** DCMP's Captioning Key
    ...
    12. **One version string.** Output names were built from three separate
    ...
    13. **Motion is isolated by absolute layout, not by attenuation.** V0.7
        ...
        smaller, not fixed it. V1.0 measures each word with the font file
        ...
        restores the V0.7 renderer for A/B. Cost: caption wrapping is now our
        responsibility, and a font substitution would desync measurement from
        rendering — the CELL 14 check exists to catch that.

    14. **Emphasis is directional.** V0.7 scored `w * |z|`, which treated a word
        ...
        V1.0 clips the emphasis features at zero so only above-average loudness,
        pitch and lengthening earn points. HNR stays two-sided, because creak
        and breathiness are both marked voice qualities.

    15. **Duration is a residual, not a raw value.** Raw duration conflates
        "this word has four syllables" with "the speaker leaned on this word".
        V1.0 fits duration against syllable count across the clip and scores the
    ...
    16. **Pitch is tracked once, with a speaker-adapted range.** V0.7 re-ran
        ...
        single halved frame into an enormous apparent range. V1.0 tracks the
    ...
    17. **A word-class prior, with a prosodic override.** Determiners,
    ...
    18. **An emphasis quota.** Even with correct scoring, a line where a third
    ...
    19. **ASR quality is a model-size problem first.** V0.7 ran whisper "base"
        with no language hint. Size dominates WER, so V1.0 defaults to large-v3
    ...
    20. **Palette hues are chosen by measurement, not by eye.** A CIEDE2000
        ...
        hue-crowding problem and the CVD problem were the same problem. V1.0's
    ...
    21. **Fearful's arousal inversion is fixed in both channels it appeared
        in.** V1.0 encoded fear at saturation 0.28 / value 0.88, which the
        ...
        high-arousal states). V1.0: (0.70, 0.78) → +0.18, above sad, and a new
    ...
    22. **The colour science self-checks or refuses to run.** ΔE2000 follows
    ...
    23. **The channel rule is resolved as an explicit, evidenced switch.**
    ...
    24. **The confidence curve stops overstating the classifier.** The legacy
    ...
    25. **The swell is volume-conserving squash-and-stretch.** Every `\fscy`
        ...
        is untouched, preserving the V0.7 A/B condition verbatim.

    26. **Reading rate is enforced, bounded, and reported.** Broadcast caption
        practice works to a characters-per-second ceiling; V1.0 extends a
    ...
    27. **Legibility parameters are visible, and substitution is a failure.**
    ...
    28. **The adjective/adverb tier ships off.** EmoLex's POS breakdown
    ...
    29. **The saturation floor is a named trade-off.** At intensity 0 every
