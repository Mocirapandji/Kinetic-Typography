...
def _(
    ...
    def enforce_min_dwell(path, seg_df, min_dwell_s, S):
        ...
        while changed:
            ...
            while i < n:
                ...
                if run_dur < min_dwell_s and (i > 0 or j < n - 1):
                    left_lab = path[i - 1] if i > 0 else None
                    right_lab = path[j + 1] if j + 1 < n else None
    ...
    def smooth_segment_emotions(seg_df, classes, mode="viterbi",
        ...
        * It cannot invent a label. Viterbi picks from what the classifier
        ...
        path = enforce_min_dwell(path, out, min_dwell_s, S)
...
def _():
    ...
    EMOTION_SMOOTH = "viterbi"   # "viterbi" | "ema" | "off"
...
def _(colorsys, np):
    ...
    return HAVE_PIL, ImageFont, assign_styles, attach_motion, pause_gap
...
def _(
    ...
    return render_long_video, seg_emotion_df


@app.cell
def _(
    angry_men_seg_emotions,
    angry_men_styled_df,
    clf_full,
    pd,
    seg_emotion_df,
    ...
    _classes_diag = list(clf_full.classes_)
    _chance = 1.0 / len(_classes_diag)

    _diag_rows = []
    for _, _r in seg_emotion_df.sort_values("segment_id").iterrows():
        _sid = int(_r["segment_id"])
        _text = str(seg_list[_sid].get("text", "")).strip()
        _row = {"segment_id": _sid, "text": _text[:55],
                "smoothed": _r["pred_emotion"],
                "raw_argmax": _r.get("pred_emotion_raw", _r["pred_emotion"])}
        for _c in _classes_diag:
            _row[_c] = _r[f"p_{_c}"]
        _diag_rows.append(_row)

    diag_df = pd.DataFrame(_diag_rows)
    _col_order = ["segment_id", "text", "smoothed", "raw_argmax",
                  "angry", "happy", "surprised", "sad", "fearful", "disgust", "neutral"]
    diag_df = diag_df[[c for c in _col_order if c in diag_df.columns]]

    diag_df["angry_vs_ahs_top"] = (diag_df[["angry", "happy", "surprised"]].max(axis=1)
                                    - diag_df["angry"]).round(3)
    diag_df["angry_above_chance"] = diag_df["angry"] > _chance

    print(f"chance level for {len(_classes_diag)} classes: {_chance:.3f}")
    print(diag_df.round(3).to_string(index=False))

    _confused = diag_df[diag_df["smoothed"].isin(["happy", "surprised"])]
    _close_call = int((_confused["angry_vs_ahs_top"] < 0.10).sum())
    _no_signal = int((~_confused["angry_above_chance"]).sum())
    print(f"\nof {len(_confused)} segments smoothed to happy/surprised: "
          f"{_close_call} had angry within 0.10 of the winner (close call), "
          f"{_no_signal} had angry below chance ({_chance:.3f}) entirely (no signal)")
    _classes_diag = list(clf_full.classes_)
    _chance = 1.0 / len(_classes_diag)

    _text_by_seg = (angry_men_styled_df.sort_values("start")
                    .groupby("segment_id")["word"]
                    .apply(lambda ws: " ".join(str(w).strip() for w in ws)))

    _diag_rows = []
    for _, _r in angry_men_seg_emotions.sort_values("segment_id").iterrows():
        _sid = int(_r["segment_id"])
        _text = str(_text_by_seg.get(_sid, ""))
        _row = {"segment_id": _sid, "text": _text[:55],
                "smoothed": _r["pred_emotion"],
                "raw_argmax": _r.get("pred_emotion_raw", _r["pred_emotion"])}
        for _c in _classes_diag:
...
def _(Path, os, process_any_video, subprocess):
    ...
    return angry_men_seg_emotions, angry_men_styled_df
