...
def _():
    # =====================================================================
    # TranscriberV13: V12 revised against the literature audit
...
def _(os, torch):
    ...
    # ---------- FIX 3 / V13.1: device is PROBED, not just queried --------
...
def _(
    ...
    # =====================================================================
    # V13.1: hand torch's cache back to the driver BEFORE CTranslate2 asks
...
def _(call, np, parselmouth, pd):
    ...
    def extract_word_features(audio_path, word_segments, count_syllables_fn):
        ...
        for i, w in enumerate(word_segments):
            ...
            # ---- spectral tilt (alpha ratio), V13.3 ----------------------
...
def _():
    ...
    SEGMENT_AROUSAL_FLOOR = 0.45     # 0.0 = V13.1 behaviour (no floor)
    AROUSAL_FEATURES = {"intensity_db": 1.0, "f0_mean": 0.6}
    AROUSAL_SPREAD = 1.5             # z at which the floor is ~76% of max

    # ----- V13.2: PER-EMOTION FLOOR BONUS (declare this if you use it) --
    ...
    # ----- V13.3: SHOUTING DETECTION AND CASE --------------------------
...
def _(colorsys, np):
    ...
    def assign_styles(words_df, seg_emotion_df, styles,
        ...
        # V13.2: the segment's absolute arousal sets a FLOOR, then the
    ...
    def yell_report(words_df, features, z_thresh, tilt_min_z=None,
        ...
        if "alpha_ratio" not in words_df.columns:
            print("YELL DIAG: no alpha_ratio column at all — these words were "
                  "extracted before V13.3. Re-run the feature extraction.")
...
def _(
    ...
    # V13.2: recover the between-segment dynamics the per-segment z-scoring
...
def _(
    ...
    def process_any_video(video_path, out_tag=None, use_bg_video=True):
        ...
        aligned = whisperx.align(result["segments"], align_model, align_meta, audio, device,
                                 return_char_alignments=False)
        # V13.1: this function loads a SECOND alignment model on top of the
...
def _(Path, os, subprocess):
    # =====================================================================
    # CELL 24b — CUT A TEST CLIP  (V13.1)
    # ---------------------------------------------------------------------
