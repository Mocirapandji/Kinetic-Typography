...
def _(
    ...
    def looks_like_loop(text, min_repeats=6, min_coverage=ASR_LOOP_MIN_COVERAGE):
        ...
        V19.1 -- two changes, both forced by BreakingBad(Happy), which this
...
def _(os):
    ...
    EMOTION_SMOOTH = "on"   # "viterbi" | "ema" | "off"
...
def _(
    ...
    #
    #  1. No pause marks. V19 put the "..." back into the control on the
...
def _(run_evaluation_batch):
    run_evaluation_batch(only=["RDR2(sad)"])
    return


@app.cell
def _(only, run_plain_only_batch):
    run_plain_only_batch(only)
    return
