...
def _(mo):
    mo.md("""
    # TranscriberV7: one variable per channel + pitch-driven motion
    ...
    emphasis. Each channel now carries exactly one thing:
    ...
    | Motion | **pitch direction** | per word |

    New in V7: motion no longer fires on an intensity threshold (which made it
    the typography channel again, just jumpier). It now follows the **f0 slope
    within each word** — rising pitch lifts, falling pitch drops, an unsteady
    contour wobbles. This is information no other channel carries, and unlike
    intensity it varies word to word *inside a single emotion*.
    """)
    return
...
def _(os):
    ...
    out_tag = "v7_" + DATASET + "_" + audio_file.split("/")[-1].replace(".wav", "")
...
def _(call, np, parselmouth):
    # =====================================================================
    # CELL 4 — CLIP-LEVEL FEATURE EXTRACTOR (split for V8 segment reuse)
    # extract_clip_features(path) behaves identically to before; CELL 5/6/13
...
def _(call, np, parselmouth, pd):
    ...
    # (roughly 0.04-0.06s at default settings). RAVDESS/IEMOCAP's curated
    # single-sentence clips never hit this, but a raw conversation has fast
    # function words, interjections, and forced-alignment artifacts that can
    # land under that floor -- and Praat raises instead of returning "no
    # pitch," which was crashing process_any_video on real dialog audio.
    # Same behaviour otherwise: an unvoiced or now-too-short word just gets
    # the same zero fallback that "no voiced frames" already used.
    # =====================================================================
    def extract_word_features(audio_path, word_segments):
        ...
        for i, w in enumerate(word_segments):
            ...
            # f0 slope (unchanged) -- naturally falls back to 0.0 when f0v is empty
            if len(f0v) >= 3:
...
def _():
    ...
    #
    #   COLOUR (hue + sat + value) -> EMOTION.        constant per clip
    #   TYPOGRAPHY (size + weight) -> INTENSITY.      the ONLY intensity channel
    #   OPACITY                    -> TIME.           the karaoke wipe
    #   MOTION                     -> PITCH DIRECTION. per word, from f0 slope
    #
    # Every channel says something the others cannot. Nothing is said twice.
    # =====================================================================

    # ----- TYPOGRAPHY channel: the sole carrier of intensity -----
    # Range is wide because size no longer competes with colour to signal
    ...
    # No trailing-voice fade: it put a SECOND variable on the opacity channel,
    # so a quiet word and an unspoken word looked alike. Loudness already feeds
    # salience, so quiet words are handled via the budget -> typography path.

    # ----- MOTION channel: pitch direction -----
    # Motion follows the f0 slope WITHIN each word (see CELL 9), not an
    # intensity threshold. Three gestures:
    #   lift   = pitch rises across the word
    #   drop   = pitch falls across the word
    #   wobble = pitch range is wide (the voice is unsteady across the word)
    # Gated by salience, so unstressed function words with steep slopes stay
    # still: a word must be prosodically prominent AND its pitch must be doing
    # something before it moves at all.
    ...
    # How the gesture is drawn.
    # "scale" (default): \fscy only. A lift makes the word swell upward off the
    #   baseline, a drop compresses it. Width never changes -> the line can
    #   never reflow, and the baseline can never break. Validated safe.
    # "tilt": \frz rotation. More literal (the word physically tips up or
    #   down), but rotation PERSISTS along an .ass line, so every word must
    #   re-declare \frz0 or the tilt leaks into the rest of the sentence and
    #   visibly breaks the baseline. Handled in CELL 18, but it is the riskier
    #   option — try "scale" first.
    MOTION_STYLE = "scale"
    ...
    #            emotion, compressed into a legible band, then separated for
    #            discriminability (sad vs fearful, disgust vs surprised).
    # italic is a CATEGORICAL marker (disgust is always italic), like the font
    # family: it identifies the emotion, it never signals emphasis.
    EMOTION_STYLES = {
...
def _(EMOTION_STYLES, subprocess):
    ...
    # wrong name never crashes — it just quietly ignores the font choice.
...
def _(
    ...
    clip_pred_vec = pd.DataFrame([clip_pred_feats])[clip_feature_cols]  # enforce column order
...
def _(np):
    ...
    def compute_salience(words_df, weights, zero_missing=frozenset(), eps=1e-9):
        ...
        |z| because standing out in EITHER direction (unusually high or
        unusually low) reads as emphasis. Within-sentence, not global, so a
        naturally high-pitched speaker doesn't get every word flagged.
        Features in `zero_missing` treat an exact 0 as "no data" (an unvoiced
        word has no pitch, which is not the same as extreme pitch).
        """
        df = words_df.copy()
...
def _(
    ...
    # intensity 0..1: 0 at fair share, 1 at FULL_DRAMA_RATIO x fair share,
    # then restrained by classifier confidence.
...
def _(
    ...
    def assign_styles_v6(words_df, emotion, styles,
        ...
        # ---- ITALIC: categorical (disgust), never intensity-driven ----
        df["italic"] = int(fam["i"])
        ...
        # ---- MOTION: pitch direction, gated by salience ----
        # A word moves only if it is prosodically prominent AND its pitch is
        # actually doing something. Unstressed function words with steep
        # slopes stay still; flat prominent words stay still too.
        def _gesture(r):
            ...
            if float(r["f0_range"]) >= wobble_range_hz:
                return "wobble"          # unsteady voice across the word
            s = float(r["f0_slope"])
            if abs(s) < slope_deadzone:
                return "none"            # essentially flat
...
def _(mo):
    ...
    - **Every word re-declares every tag it uses.** .ass override tags persist
      along a Dialogue line until something overrides them, so a word that
      animates its scale or rotation must be followed by words that explicitly
      reset those tags — otherwise the effect leaks down the sentence. This is
      why `
    scx100
    scy100
    rz0` appears in every word's tag block.
    - **`
    scy` swell is the default motion**, because it cannot break the
      line: width never changes (no reflow) and the baseline stays flat.
      `
    rz` tilt is more literal but rotation propagates aggressively along
      an .ass line and visibly breaks the baseline if any word forgets to
      reset it. Both are implemented; MOTION_STYLE picks.
    - **There is no per-word position tag in .ass.** `\pos` and `\move` are
      line-level, like `
    ad`. So a "lift" is a swell or a tilt, never a true
      translation. This is a format constraint, not a design choice.
    """)
    return
...
def _(
    ...
    def render_budget_video(audio_path, df, emotion, styles, out_dir="outputs",
        ...
        def word_tags(rw, line_start):
            # COLOUR + TYPOGRAPHY, set once, never animated.
            # \fscx100\fscy100\frz0 are RESETS: .ass tags persist along a line,
            # so without these a previous word's swell or tilt would leak into
            # this one.
            ...
            # opaque over its own spoken interval, so the opacity front travels
            # across the line with the voice. Every word ends at alpha 00.
            ...
            # MOTION: the word follows its own pitch contour.
            ...
            if g != "none" and k > 0.0:
                ...
                if motion_style == "scale":
                    # \fscy scales from the BASELINE, so growing it makes the
                    # word swell upward (a lift) and shrinking sinks it (a drop).
                    # Width is untouched -> no reflow, baseline stays flat.
                    if g == "lift":
        ...
        if caption_mode == "word":
            # legacy: one word at a time, centred
            ...
            for si, sid in enumerate(seg_ids):
                ...
                # linger past the last word, stay readable, never overlap next line
    ...
    v7_video, v7_ass = render_budget_video(
    ...
    print("Wrote:", v7_video, "and", v7_ass)
    return


@app.cell
def _(emotion_color, extract_clip_features_from_sound, np, parselmouth, pd):
    # =====================================================================
    # CELL 19 — PER-SEGMENT EMOTION (new)
    # CELL 13 predicts ONE emotion for the whole file. That's correct for
    # RAVDESS (one sentence = one file) but wrong for a real conversation,
    # where the emotion drifts sentence to sentence. Same classifier
    # (clf_full), same 14 features -- run once per WhisperX segment on that
    # segment's own audio slice instead of once for the whole file.
    # =====================================================================
    def predict_segment_emotions(audio_path, segments, clf, feature_cols):
        ...
        for sid, seg in enumerate(segments):
            ...
            try:
                ...
                feats = extract_clip_features_from_sound(seg_snd)
                vec = pd.DataFrame([feats])[feature_cols].to_numpy()
                ...
                # segment too short/silent for Praat -- fall back to neutral
                # rather than crashing a 40-line conversation over one bad turn
                print(f"  segment {sid} ({s0:.2f}-{s1:.2f}s) skipped: {e}")
                pred, p_top, conf_scale = "neutral", 0.0, 0.5
            rows.append({"segment_id": sid, "start": s0, "end": s1,
                         "pred_emotion": pred, "p_top": round(p_top, 3),
                         "conf_scale": round(conf_scale, 3)})
        return pd.DataFrame(rows)

    # =====================================================================
    # CELL 20 — MOTION (unchanged maths, pulled out of assign_styles_v6 --
    # pitch direction never needed to know which emotion won) + STYLE
    # MAPPING v8: colour/font/anim now vary PER SEGMENT, not once per clip.
    # =====================================================================
    ...
    def assign_styles_v8(words_df, seg_emotion_df, styles, base_font, font_swing, bold_thresh):
        ...
        # CELL 16's intensity_raw is the budget share ONLY. The single
        # clip-wide conf_scale multiply from CELL 16 is replaced here with
        # each word's OWN segment's conf_scale -- a confidently-classified
        # sentence styles at full strength, a shaky one gets damped, sentence
        # by sentence rather than for the whole file at once.
    ...
    return assign_styles_v8, attach_motion, predict_segment_emotions


@app.cell
def _(json, os, subprocess):
    # =====================================================================
    # CELL 21 — RENDER onto a REAL video, colour/font allowed to change from
    # one caption line to the next (because emotion is per-segment now).
    # =====================================================================
    ...
    def render_long_video(audio_path, df, styles, out_dir="outputs",
                          ...
                          motion_style="scale", swell_peak=30, tilt_deg=7,
                          motion_min_ms=200, motion_max_ms=700,
                          motion_tempo=None, bg_video_path=None, tag="v8_demo"):
        """
        Same tag mechanics as V7's render_budget_video: every word resets
        \\fscx100\\fscy100\\frz0, opacity is a per-word wipe, motion is a
        swell/tilt gated by strength. Two things are new:
          1. colour/font/anim are read PER WORD from df, not from one fixed
             `emotion` argument -- a long clip has more than one.
          2. bg_video_path: if given, THAT video is the picture. If it
             already has its own audio, that's kept; if not, the audio you
             ran whisperx/parselmouth on gets muxed in. If bg_video_path is
             None, this falls back to V7's black screen exactly as before.
        """
        os.makedirs(f"{out_dir}/ass", exist_ok=True)
        os.makedirs(f"{out_dir}/video", exist_ok=True)
        ...
        if bg_video_path:
            ...
            # footage can run past the last word (trailing silence, a second
            # speaker) and shouldn't get truncated.
            if bg_has_audio:
...
def _(
    ...
    np,
    os,
    predict_segment_emotions,
    render_long_video,
    subprocess,
    whisperx,
):
    # =====================================================================
    # CELL 22 — "INSERT ANY VIDEO": one call, full pipeline, real video out
    # Needs CELLS 0-13 already run once (clf_full, asr_model, clip_feature_cols
    # must exist).
    # =====================================================================
    def process_any_video(video_path, out_tag=None, use_bg_video=True):
        ...
        seg_emotion_df = predict_segment_emotions(extracted_audio, seg_list, clf_full, clip_feature_cols)
...
def _(iemocap_dir, os, process_any_video):
    # =====================================================================
    # CELL 23 — DEMO: pull a whole conversation straight off your IEMOCAP
    # drive instead of needing a brand-new video file
    # =====================================================================
    def find_iemocap_dialog_video(utt_id, iemocap_dir):
        """
        IEMOCAP's full dialog recordings (both actors, the real multi-turn
        conversation) live at {Session}/dialog/avi/DivX/{dialog}.avi -- same
        `dialog` derivation CELL 1 already uses for _dialog.
        ASSUMPTION to verify: your IEMOCAP_full_release still has dialog/avi/
        (it ships in the standard release; if yours doesn't, this returns
        None and you fall back to use_bg_video=False, i.e. black screen).
        """
    ...
    _demo_utt = "Ses01F_impro01_F012"     # any utterance id from the dialog you want
...
def _(os, process_any_video):
    ...
    # Not RAVDESS, not IEMOCAP -- a real film clip sitting in the project
    # root. process_any_video() doesn't care what genre/source a video is,
    # it just needs speech in it. use_bg_video defaults to True, so this
    # burns onto 12AngryMenTest.mp4 itself, not a black screen.
    # =====================================================================
    new_video_path = "12AngryMenTest.mp4"   # relative to wherever marimo was launched from
...
def _(mo):
    mo.md("""
    ### V8: any video in, real background out, per-segment emotion

    Everything up to V7 assumed one file = one sentence = one emotion, because
    that's true of RAVDESS and of IEMOCAP's per-utterance wav cuts. Feeding
    the pipeline a real video — a full conversation, or literally any clip a
    user drops in — breaks that assumption, so V8 is one coherent change
    (emotion becomes a per-segment property) plus two robustness fixes that
    change surfaced.

    11. **Emotion is predicted per segment, not per clip.** CELL 13's
        clip-level prediction is correct for RAVDESS but wrong for a
        conversation, where the emotion changes turn to turn.
        `predict_segment_emotions` reuses the exact same classifier
        (`clf_full`) and the same 14 clip-level features from CELL 4/6 —
        nothing about the model or its features changed, only its
        granularity: it now runs once per WhisperX segment, on that
        segment's own audio slice, instead of once for the whole file.
    12. **Colour/font/italic/anim now vary by segment, not by clip.**
        `assign_styles_v8` looks each word's emotion up from its segment via
        `segment_id`, replacing the single clip-wide `emotion` argument
        `assign_styles_v6` took. Motion (`attach_motion`) needed no change at
        all — it already never touched the emotion label, which is a nice
        confirmation of decision 7: pitch direction really is independent of
        which emotion won.
    13. **Per-segment confidence replaces per-clip confidence.** CELL 16's
        `conf_scale` was one number for the whole file. In `assign_styles_v8`
        it's looked up per segment instead, so a confidently-classified
        sentence styles at full strength and a shaky one gets damped —
        sentence by sentence, not for the whole conversation at once.
    14. **Rendering can burn onto a real video, not just a black screen.**
        `render_long_video` takes an optional `bg_video_path`. If given, that
        video is the picture: its own audio is kept if it has one, otherwise
        the audio the pipeline transcribed gets muxed in. Output duration
        comes from the video file itself (via ffprobe), not from the last
        transcribed word plus padding, so real footage never gets truncated.
        `bg_video_path=None` reproduces V7's black-screen behaviour exactly,
        so the original single-clip demo still runs unchanged.
    15. **The existing dataset can supply that video directly, without a new
        source file.** `find_iemocap_dialog_video` derives IEMOCAP's full
        dialog recording path from an utterance id (same `dialog` derivation
        CELL 1 already used). This also means the RAVDESS-trained classifier
        is now being asked to label natural conversational turns it never
        saw in training — the same acted-vs-spontaneous gap the
        RAVDESS/IEMOCAP framing already names (CELL 1), now visible in the
        render itself and not only in a held-out accuracy number.
    16. **Per-word prosody extraction was silently assuming every word is
        "long enough."** `to_pitch()` and `to_harmonicity()` both need a
        minimum window length (roughly 0.04-0.06s at default settings) and
        raise a `PraatError` rather than returning "no pitch" when a word is
        shorter than that. RAVDESS/IEMOCAP's curated single-sentence clips
        never produced a word that short; a raw conversation's fast function
        words, interjections, and forced-alignment artifacts do.
        `extract_word_features` now catches that error per word and falls
        back to the same zero values already used for "unvoiced," instead of
        crashing the whole file over one bad word.

    *Implementation note: `get_video_info` (decision 14) needs `import json`
    in CELL 0 — the only new import V8 requires.*
    """)
    return
