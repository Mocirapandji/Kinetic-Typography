...
def _(
    ...
    def looks_like_loop(text, min_repeats=6, min_coverage=0.6):
        """Whisper's classic failure: one phrase emitted N times in a row.

        V19.1: two changes, both from BreakingBad(Happy), which was killed
        by this function reading a real line as a hallucination.

        The transcript was "$672,000. All of it? No. Each. Each. Each.
        $672,000 each. Each. Yes! Hell yeah!..." — LOOP_WORD_RE strips
        digits, so "$672,000" tokenises to NOTHING and the five "each"es
        collapsed into one unbroken run of 5, clearing the old threshold
        of 4. The segment was dropped, word_df came out empty, and
        compute_salience raised KeyError: 'Column not found: word' two
        stages downstream of the cause.

        min_repeats 4 -> 6: genuine Whisper loops run 8-20+ repeats.

        min_coverage: a real hallucination loop IS the segment; genuine
        repetition sits inside other speech. Here the repeated gram was
        5 of 23 tokens (22%), so coverage separates the two cases on a
        structural property rather than on a threshold tuned to one clip.
        """
...
def _(ASR_LOOP_MIN_REPEATS, run_evaluation_batch):
    # =====================================================================
    # CELL 27 — RE-RUN ONE STIMULUS  (BreakingBad(Happy), loop-screen fix)
    # ---------------------------------------------------------------------
    # BreakingBad(Happy) failed the v18 batch with KeyError: 'Column not
    # found: word'. That error is two stages downstream of the cause. The
    # transcript was fine:
    #
    #   "$672,000. All of it? No. Each. Each. Each. $672,000 each. Each..."
    #
    # LOOP_WORD_RE strips digits, so "$672,000" tokenises to NOTHING and
    # the five "each"es collapse into one unbroken run. looks_like_loop saw
    # 5 >= ASR_LOOP_MIN_REPEATS(4), dropped the only segment, and the clip
    # reached compute_salience with an empty word_df.
    #
    # Fixed by raising ASR_LOOP_MIN_REPEATS to 6 in CELL 1b. Genuine
    # Whisper hallucination loops run 8-20+ repeats, so the screen still
    # does its job. Recorded here because a threshold tuned against one
    # clip is exactly the kind of change that needs its reason written
    # down next to it.
    #
    # skip_existing=False is required: without it the runner would see the
    # other six pairs already on disk and skip them, but this clip has NO
    # outputs, so `only=` is what actually restricts the work.
    # =====================================================================
    REDO_STEM = "BreakingBad_Happy"

    if ASR_LOOP_MIN_REPEATS < 6:
        print(f"ASR_LOOP_MIN_REPEATS is still {ASR_LOOP_MIN_REPEATS} — edit "
              f"CELL 1b to 6 first, or this will drop the segment again.")
        redo_manifest = None
    else:
