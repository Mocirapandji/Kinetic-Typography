...
def _(
    ...
    def smooth_segment_emotions(seg_df, classes, mode="viterbi",
        ...
        * It cannot invent a label. Viterbi picks from what the classifier
        ...
        print(f"SMOOTH ({mode}, self_bias={self_bias}): {_n_sw}/{len(out)} "
              f"segment labels changed | colour changes {_runs_before} -> "
              f"{_runs_after}")
        if _runs_after <= 1 and len(out) > 4:
            print("  WARNING: the whole clip collapsed to ONE emotion. That "
                  ...
                  "not self_bias set so high that the classifier no longer "
                  "matters.")
        return out
...
def _():
    ...
    EMOTION_SELF_BIAS = 0.55     # P(stay in the same emotion) per segment
...
def _(
    ...
    seg_emotion_df = smooth_segment_emotions(
        ...
        self_bias=EMOTION_SELF_BIAS, confidence_scale_fn=confidence_scale)
    seg_emotion_df
