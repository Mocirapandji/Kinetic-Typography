...
def _():
    ...
    # TranscriberV19.1
    # ---------------------------------------------------------------------
    # V19 plus the three fixes that came out of the BreakingBad(Happy)
    # failure. All three are marked "V19.1" in the code so they can be
    ...
    #      The code has been V19 since the strict/tiered shout work; the tag
...
def _(mo):
    mo.md(r"""
    # TranscriberV19.1: readable lines on rapid speech

    V19.1 is V19 with the three fixes that came out of one stimulus,
    `BreakingBad(Happy)`, failing in two different ways.
    ...
    **The version tag was a version behind.** Outputs were labelled `v18` by
    code that had been V19 for two revisions.
    """)
    return
...
def _(os, torch):
    ...
    #
    # V19.1: "v18" -> "v19". The code has been V19 since the strict/tiered
...
def _():
    ...
    ASR_DROP_LOOPS = True
    # V19.1: 4 -> 6. BreakingBad(Happy) transcribed as
...
def _(
    ...
    # V19.1: minimum share of a segment the repeated gram must occupy
    ...
    def looks_like_loop(text, min_repeats=6, min_coverage=ASR_LOOP_MIN_COVERAGE):
        ...
        V19.2 -- one more change, forced by the CELL 9e self-test, which
        this function still failed after V19.1: it never caught
        "Subscribe to my channel." repeated six times.

        V19.1's loop only tried gram sizes (1, 2, 3). "Subscribe to my
        ...
        V19.2: the gram sizes to try are now derived from the data. The
        ...
        V19.1's two earlier changes are unchanged and still do their job:
        ...
        if len(toks) < min_repeats:
            return False
        # V19.2: try every unit size that could possibly repeat
...
def _(
    ...
    #
    # V19.1 (marimo): this is deliberately NOT its own cell. It REASSIGNS
...
def _(ASR_LOOP_MIN_COVERAGE, ASR_LOOP_MIN_REPEATS, looks_like_loop):
    # =====================================================================
    # CELL 9e — LOOP SCREEN SELF-TEST  (V19.1, new)
    # ---------------------------------------------------------------------
...
def _(call, np, parselmouth, pd):
    ...
    def extract_word_features(audio_path, word_segments, count_syllables_fn):
        ...
        if df.empty:
            # V19.1: an empty frame here is what turned a dropped segment
...
def _(os):
    ...
    # V19 — BACK TO STRICT, AND TIERED  (reverts the V18 widening)
    # ---------------------------------------------------------------------
    # V18 shipped EXCLAIM_EMOTIONS = {angry, frustrated, excited, happy} at
    ...
    # being reverted -- deliberately, not by accident. The V18 note below
    ...
    #
    # The new ask is narrower than the pre-V18 state, so the emotion set is
    ...
    # was the single biggest source of stray capitals in V18.
    #
    ...
    # including it is most of what made V18 shout constantly. Someone
    ...
    # V19: "strict" is the new default and the new entry in the table.
    ...
    # ----- V19: the "super super" bar for SHOUT_EXTREME_EMOTIONS -------
    ...
    # ----- V19: raised-voice corroboration -----------------------------
    ...
    # ----- V19: the torn / "close to mad" clause, now OFF --------------
    # V18's own comment: "At trigger_happy the floor is barely above
    ...
    # ----- V19: hard cap on how much of a clip may shout ---------------
    ...
    # ----- V19: does the ACOUSTIC yell detector get an emotion veto? ---
    # Read the V15 note at the top of this section first, because this
    ...
    # False = V15/V18 behaviour: measured vocal effort is sufficient.
    #
    ...
    # ----- V19.1 CHANNEL 4a: MERGE SEGMENTS TOO SHORT TO READ ----------
    ...
    # ----- V19 CHANNEL 4b: SILENCE AS PUNCTUATION AND AS A LINE BREAK ---
...
def _(colorsys, np, re):
    ...
    def apply_emotion_shout(styled_df, emotions, blend_margin,
        ...
        the acoustic yell detector (CELL 17/18). V19 — strict and tiered.
        ...
        V19 changes, in order of how much they reduce the fire rate:
        ...
        3. TORN CLAUSE OFF. Capitalising on the strength of a RUNNER-UP
           was doing most of V18's widening. Colour degrades gracefully
        ...
        V19.1 ordering note: this runs BEFORE merge_rapid_segments, so a
        ...
        # V19: when nothing fires, say WHICH gate stopped what -- the same
    ...
    # ---------------------------------------------------------------------
    # V19.1 — MERGE SEGMENTS TOO SHORT TO READ
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # V19 — SILENCE AS PUNCTUATION, AND AS A LINE BREAK
    # ---------------------------------------------------------------------
    ...
    def split_on_long_pauses(df, enable=True, dots_min_s=0.28,
        ...
        V19.1: runs AFTER merge_rapid_segments, so the ids it reads are
...
def _(merge_rapid_segments, pd):
    # =====================================================================
    # CELL 13d — MERGE SELF-TEST  (V19.1, new)
    # ---------------------------------------------------------------------
...
def _(classify_words, np, pd):
    ...
    def compute_salience(words_df, weights, zero_missing=frozenset(),
        ...
        V19.1: the empty-frame guard. BreakingBad(Happy) reached here with
...
def _(
    ...
    # V19.1: MERGE FIRST, SPLIT SECOND. merge_rapid_segments joins captions
    ...
    # V19: silence -> ellipsis, and past a longer threshold -> a new
...
def _(PAUSE_HOLD, PAUSE_HOLD_THRESH, pd, styled_word_df):
    ...
    def styling_coverage(styled_df, label="clip"):
        ...
        _stats = {
            ...
            # V19.1: caption LINES, not just words. The BreakingBad failure
...
def _(mo):
    ...
    **V19.1 — read the clamp in `build_ass_events` before touching timing.**
...
def _(
    ...
    def build_ass_events(rows, width, height, tags_fn, layout_mode,
        ...
        for si, sid in enumerate(seg_ids):
            ...
            # V19: a line that was cut at a long pause holds until the line
            ...
            if "line_hold_until" in seg.columns:
                ...
                if _hold > 0.0:
                    e0 = max(e0, _hold)
            # ---- THE CLAMP (V19.1: read the note before changing timing) --
            ...
            # V19.1: a caption too brief to read is a DIFFERENT failure
        ...
        # V19.1: the report that would have caught BreakingBad(Happy) on
...
def _(
    ...
    def process_any_video(video_path, out_tag=None, use_bg_video=True,
        ...
        # V19.1: fail HERE, where the cause is visible, rather than three
        ...
        if EXCLAIM_ENABLE:
            styled_df = apply_emotion_shout(
                ...
                max_frac=SHOUT_MAX_FRAC,
                yell_emotion_gate=YELL_EMOTION_GATE)
        # V19.1: merge first (join captions too brief to read), split
        ...
        styled_df = merge_rapid_segments(
            ...
            max_gap_s=MERGE_MAX_GAP_S, max_chars=MERGE_MAX_CHARS,
            min_line_s=MERGE_MIN_LINE_S)
        # V19: silence -> ellipsis / new subtitle line. Last writer of
...
def _(
    ...
    # =====================================================================
    # CELL 25 — RUN ON ONE OF THE REAL SOURCE VIDEOS  (V19)
    # ---------------------------------------------------------------------
    ...
    #
    # V19.1: TEST_CLIP_ENABLE. This cell is the single most expensive thing
...
def _(Path, VERSION_TAG, json, os, re, subprocess):
    ...
    # ---------------------------------------------------------------------
    # Split out of the EmoLex cell for V19.1. These are constants and pure
    ...
    # here. V19.1 bumped it to "v19", so skip_existing will not find the
...
def _(EMOTION_STYLES, PAUSE_DOTS, PAUSE_DOTS_SKIP_ON_BANG, add_pause_ellipsis):
    ...
    def flatten_to_plain(styled_df, base_font, spec=PLAIN_SPEC):
        ...
        V19.1 note: segment_id has already been through
        ...
        if "word" in out.columns:
            out["word_display"] = out["word"].astype(str)
        # V19: ...except the pause ellipsis, which is put back. The dots
        ...
        for _i, _vid in enumerate(_vids, 1):
            ...
            try:
                ...
                _rows.append({
                    ...
                    # V19.1: caption LINES after merge+split, which is what a
...
def _(mo):
    ...
    **V19.1 bumped `VERSION_TAG` to `"v19"`**, so `skip_existing` will not
...
def _(ASR_LOOP_MIN_REPEATS, run_evaluation_batch):
    # =====================================================================
    # CELL 27 — RE-RUN ONE STIMULUS  (V19.1)
    # ---------------------------------------------------------------------
