...
def _(mo):
    ...
    - **Every word re-declares every tag it uses.** .ass override tags persist
      ...
      why `    scx100    scy100    rz0` appears in every word's tag block.
    ...
    - **There is no per-word position tag in .ass.** `\pos` and `\move` are
      line-level, like `    ad`. So a "lift" is a swell or a tilt, never a true
...
def _(mo):
    ...
    The system has four visual channels. Versions up to V5 had **all four
    encoding intensity at once** — a loud word arrived bigger, bolder, darker,
    more saturated AND bouncing. Five cues for one variable reads as noise, not
    emphasis, and it is the single reason the early renders looked bad. Each
    channel now carries exactly one thing, and nothing is said twice:
    ...
    1. **Colour is constant across a clip.** Valdez & Mehrabian measured how
       *a colour* makes a viewer feel, which maps to the *emotion* — not to
       ...
       fix each one's saturation and value, then compressed into a legible band
       and separated for discriminability (sad vs fearful, disgust vs
    ...
    2. **Hue palette from survey data.** Jonauskaite et al. 37-nation survey
       ...
       128-year systematic review, and it keeps sad and fearful discriminable
       in the mute test) and surprised-orange (surprise is absent from the
       Geneva Emotion Wheel, so it rests on review counts alone — the weakest
       anchor in the palette).
    3. **Legibility constraints are documented, not hidden.** Fear's
       evidence-based colour is black (48%), unreadable burned into video. A
       white caption plate was trialled (BorderStyle=4) and rejected as
       visually intrusive; fear now takes a desaturated cold blue-violet. The
       palette is therefore a *constrained* empirical selection, not a naive
       argmax, and the constraint is legibility.
    ...
    6. **The trailing-voice fade was built, then removed.** Quiet words
       ...
       alike, exactly the confusion this redesign exists to eliminate. Loudness
       already feeds salience, so quiet words are handled through the budget ->
       typography path. Worth revisiting on a channel that is genuinely free.
    7. **Motion encodes pitch direction, not intensity again (V7).** In V6,
       motion fired on an intensity threshold, which made it the typography
       channel with extra steps. It now follows the f0 slope within each word
       (semitones/sec, so speaker- and duration-independent): rising pitch
       lifts, falling pitch drops, a wide pitch range wobbles. Motion is gated
       by salience so unstressed function words stay still. This channel now
       carries information no other channel has, and it varies word to word
       *within* a single emotion — which the clip-constant colour cannot.
    8. **Swell (    scy) is the default gesture, tilt (    rz) is opt-in.**
       .ass has no per-word position tag (\pos and \move are line-level, like
       ad), so a "lift" must be built from scale or rotation. Swell cannot
       reflow the line (width is untouched) and cannot break the baseline;
       rotation propagates along a line and visibly breaks the baseline unless
       every subsequent word resets     rz0. Both are implemented; the safe one
       is the default.
    9. **Every word re-declares every tag it uses.** Override tags persist
       along an .ass Dialogue line, so animated scale and rotation are reset
       explicitly on each word. This bug (a tilt leaking down the sentence and
       sloping the baseline) was found by rendering and inspecting frames, not
       by reading the spec.
    10. **Everything upstream is untouched.** The classifier, the budget, and
        prosody extraction are unchanged since V3, apart from the addition of
        f0_slope. Every version since has only re-mapped emotion + intensity
        onto visuals.
    """)
    return
